...
- `scripts/workflow.py`：管理 `.agent_state/worktrees/state.json`、`.agent_state/worktrees/merge_queue.json`、reports path、lane worktree 建立、主 checkout preview merge 與 final fast-forward。
...
固定 exit code：`0` 成功、`10` schema invalid、`20` queue not head、`30` lock timeout、`40` invalid transition / conflicting input、`50` git operation failed 或 postcondition failed。加 `--json` 可輸出 machine-readable result。
...
- 建立主 checkout 驗收預覽時，用 `workflow.py preview start <task-id> --requested-by <agent-id>`。script 會登記 queue、確認 task 是 queue 第一個 entry、記錄 target commit / base head、執行 `git merge --no-commit --no-ff <target-commit>`，並驗證 `MERGE_HEAD`。
...
- 完成主線收尾時，用 `workflow.py final fast-forward <task-id> --requested-by <agent-id>`。script 會在同一個 critical section 內處理仍開著的 preview、執行 `git merge --ff-only <target-commit>`、驗證 `HEAD`，成功後釋放 queue 並關閉 task entry。
- `preview start` 與 `final fast-forward` 只在主 checkout 目前 branch 等於 task 的 `base_branch` 時執行；branch 不符時 fail fast。
- `preview start` 與 `final fast-forward` 要求 tracked files 乾淨；main checkout 可保留 untracked 或 ignored-untracked files，例如 local report 或 scratch note。若 untracked path 會和 target commit 的變更路徑重疊，script 會 fail fast，並用 Git 的 `--no-overwrite-ignore` 保護 ignored files，避免 merge 覆蓋或卡在 Git 的 untracked-file protection。
...
6. 回到主 checkout 的 `base_branch` 前，用 `workflow.py preview start <task-id> --requested-by <agent-id>` 建立未提交的驗收預覽。script 會取得 queue、記錄 target commit、執行 preview merge 並驗證 postcondition；只有 queue 第一個 entry 可建立 preview。這個 merge 只作 preview；不要 commit，也不要在主 checkout 的 merge preview 上直接改碼。
...
8. 若用戶沒有改動意見並授權收尾，用 `workflow.py final fast-forward <task-id> --requested-by <agent-id>` 把 `agent/<task-id>` 接到主線，保持 `base_branch` 線性歷史。若 preview 仍開著，script 會先驗證並取消該 preview，再執行 final fast-forward；成功後 script 釋放 queue 並從 `state.json` 移除 task entry。接著結束 Phase，移除 task 的所有 lane / integration worktree 並刪除對應 branch：
...
9. 若尚未建立 preview，就直接用 `workflow.py final fast-forward <task-id> --requested-by <agent-id>` 取得 queue 並收尾；不要為了收尾建立 merge commit。
...
5. linear preview / 收尾：建立 worktree 時用 `workflow.py worktree create-lane` 或 `workflow.py task create` / `lane create` 記錄當前目標 branch / commit；Phase 變更完成後，先整理各 lane branch，再依序 fast-forward 到 parent integration branch `agent/<task-id>`。回主 checkout 建立驗收 preview 時用 `workflow.py preview start`；用戶有改動意見時用 `workflow.py preview abort` 釋放 queue，再委派 sub-agent 回相關 lane / integration worktree 修改；用戶無意見並授權收尾時用 `workflow.py final fast-forward` 進主線並關閉 task entry，結束 Phase，移除所有 lane / integration worktree 並刪除 branch。未授權進入 preview 時停在可檢查 diff / branch commit，回報 task-id、lane-id、worktree path 與下一步。
...
7. **關閉 task / lane worktree**：Phase / task item 收尾時，先在各 lane branch rebase / 整理 commit，必要時整合到 parent branch `agent/<task-id>`，再用 `workflow.py preview start` 建立主 checkout 驗收預覽。用戶有改動意見就用 `workflow.py preview abort` 釋放 queue，並讓 sub-agent 回相關 lane / integration worktree 修；無意見且授權收尾就用 `workflow.py final fast-forward` 進主線並關閉 task entry，移除所有 lane / integration worktree 並刪除 branch。若 task 已告一段落且沒有剩餘 backlog，可以先詢問用戶是否關閉；用戶同意後就按同一套清理流程移除當前 worktree 並歸檔 plan。若不整合，清理後用 `workflow.py task close` 移除 entry；只有仍在 active/reviewing/merge_preview/blocked 的 task 才保留 entry。task 結束後，將 `.agent_state/plans/<task-id>/` 整包移到 `.agent_state/plans/archives/<task-id>/`，讓 plans 根目錄維持乾淨。
