...
- 取消主 checkout 驗收預覽時，用 `workflow.py preview abort <task-id>`。script 會確認 queue head 與 `MERGE_HEAD` 指向同一個 target commit，執行 abort，驗證主 checkout 回到 recorded base head 且 worktree 乾淨，成功後才釋放 queue。
