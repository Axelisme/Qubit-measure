...
def T1_analyze(x: float, y: float, return_err=False):
    y = np.abs(y)
    ...
    t1_str = f"{t1:.2f}us \pm {err[2]*1e3:.2f}ns"
...
def T2fringe_analyze(x: float, y: float, return_err=False):
    y = np.abs(y)
    ...
    t2f_str = f"{t2f:.2f}us \pm {err[4]*1e3:.2f}ns"
...
def T2decay_analyze(x: float, y: float, return_err=False):
    y = np.abs(y)
    pOpt, pCov = ft.fitexp(x, y)
    t2e = pOpt[2]
    err = np.sqrt(np.diag(pCov))

    t2e_str = f"{t2e:.2f}us \pm {err[2]*1e3:.2f}ns"
