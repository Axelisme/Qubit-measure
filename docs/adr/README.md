...
- [0032 — Arbitrary waveform data uses a reference time axis](0032-arbitrary-waveform-reference-time-axis.md)：Arbitrary waveform playback data 帶 reference time axis，不依 ML arb length 做 stretch/compress；program layer 在 requested playback window 上取樣，若 length 短於 data time-axis end 則截斷並 log warning。
