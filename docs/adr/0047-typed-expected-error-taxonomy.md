...
shared remote dispatch以nominal `ExpectedError`作為generic projection的唯一gate，並把
`INVALID_INPUT`投影到既有`INVALID_PARAMS`、`FAILED_PRECONDITION`投影到既有
`PRECONDITION_FAILED`。projection原樣傳遞message與reason，generic data固定為`None`。
handler直接raise的`RemoteError`優先穿透，因此request coercion與structured mapping（例如
`ArbWaveformError.data`）維持原 owner。

main/off-main dispatch使用同一translator。ordinary exception由controller-error branch記錄
traceback；translator本身若因非法category失敗，同樣進controller-error branch，不留下空reply
state。wire envelope、code、reason、data與版本皆不改變。
...
- caller-correctable semantics由producer擁有，Qt view與remote adapter可共享同一分類；transport
  projection集中於shared dispatch。
