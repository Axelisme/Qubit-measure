...
- **op 專屬**的 `stop_event → schedule_stop_scope(StopSignal(...))` 由 **run policy** 的 work thunk 接——experiment runner 的 cancellation 語義只留在 run policy 一處，不再外洩 bg。一般 experiment 與 executor workflow 都透過 `Schedule` 讀同一個 ambient stop scope，避免 GUI cancel 與底層執行路徑分叉。
