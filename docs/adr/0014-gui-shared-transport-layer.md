...
`McpBridge` 已有 transport seam：注入 `Transport` Protocol（真實實作 `SocketTransport`，測試以 `FakeTransport` 經 `set_transport` 注入），socket I/O 退到接縫之後。剩餘未做：把 version-guard + op-handle 簿記從 measure-gui server 的 module global（`_LAST_SEEN` / `_OP_BY_KEY`）移到 bridge instance 上，以退役 remote 測試裡約 20 處 monkeypatch。列為未來工作，不阻塞本階段。
