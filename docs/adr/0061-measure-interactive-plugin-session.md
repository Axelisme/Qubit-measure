...
- The flux frontend owns Matplotlib artists, pointer selection, timers and preview. Move only previews a candidate; valid release recalculates on the latest session snapshot and commits. An external commit, Esc, focus loss or hide drops the preview. Local Auto Align captures numeric inputs for background computation and commits on the owner loop. It is single-flight, and pending work does not prevent Done. Terminal delivery is discarded.
- Measure changes its drag gesture to preview/release commit. Notebook `InteractiveLines` retains its existing immediate-drag behavior and does not join the measure session. Both use `analysis.fluxdep.line_state` for numerical rules.

## Consequences

A GUI can show local preview while remote reads committed state; the remote response must identify `preview_active` as presentation metadata, not as another analytical state. The GUI-side wire handler for command discovery, validation and projection is a separate follow-up; this ADR does not add an MCP bridge route or change the generic RPC channel ([[0059]], [[0060]]). There is no plugin discovery, undo history, conflict merge or session persistence.
