...
`{role_id: (blank, ref)}`。Inspect/catalog 用 `.blank`；Builder 的 `.role()` 以 `Init` enum 選擇：
`Init.ADOPT`（預設）用 `.ref`（library aware，無 ref variant 時退回 `.blank`）、`Init.INLINE` 強制
`.blank`（守 ignore_library_readout）、`Init.DISABLED` 走 `.ref(ctx, optional=True)`（library
...
- 映射規則：`make_<role>_default` → `.role(path, role, Init.INLINE)`；`make_<role>_ref_default(ctx)`
  → `.role(path, role)`（`Init.ADOPT` 預設）；`make_<role>_ref_default(optional=True)` →
  `.role(path, role, Init.DISABLED)`；
