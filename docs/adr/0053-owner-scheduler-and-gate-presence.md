...
class OwnerScheduler(Protocol):
    def is_owner_thread(self) -> bool:
        """回報caller是否為State owner thread；thread-safe。"""
...
- `is_owner_thread()`是正式contract，不是adapter私有能力：executor teardown在owner上不可阻塞等待
  queued terminal delivery，必須Fast Fail回傳讓caller pump後重試。
