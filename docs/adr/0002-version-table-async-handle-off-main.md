...
- device/run/connect 啟動回 `operation_id`（= token）；terminal slot 帶 outcome release。
...
- **mcp = policy + 簿記 + 翻譯**：持 `last_seen`、知道每操作依賴哪些資源、組 `expected_versions`、收 `PRECONDITION_FAILED`；持「語義 key→最新 operation_id」對照（device→name / run→tab_id / connect→soc）。版本號與 operation_id **只在 RPC↔mcp 之間流動**，從回傳 strip 掉。
- **agent（LLM）= 只收語義**：
  ...
  - async 完成 = **poll / wait 操作句柄**：`gui_run_poll` / `gui_device_poll` / `gui_soc_connect_poll`（非阻塞）；wait 回 `{status: finished|timed_out|no_operation, waited_seconds}`，timeout 不 raise。`gui_device_wait_operation(name)` 內部翻 operation_id 發 `operation.await`。
