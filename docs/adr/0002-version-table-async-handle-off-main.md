...
- **agent（LLM）= 只收語義**：
  ...
  - async 完成 = **poll / wait 操作句柄**：`gui_run_poll` / `gui_device_poll` / `gui_connect_poll`（非阻塞）；wait 回 `{status: finished|timed_out|no_operation, waited_seconds}`，timeout 不 raise。`gui_device_wait_operation(name)` 內部翻 operation_id 發 `operation.await`。
