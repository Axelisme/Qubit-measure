...
python script/start_server.py --port xxx --soc v2
```

這將啟動一個 Pyro4 伺服器，使硬體控制物件可供遠端客戶端使用。
...
* **磁通依賴與能階擬合：`notebook/analysis/fluxdep_fit.ipynb`**  
  ...
  * 使用事先透過 `script/generate_fluxonium_sample.py` 產生的 Fluxonium 模擬資料庫（預設存於 `Database/simulation/fluxonium_*.h5`，內含隨機掃描的 \((E_J, E_C, E_L)\) 參數與對應能階），搭配 `search_in_database` 與 `fit_spectrum` 搜尋最佳的 \(E_J, E_C, E_L\) 參數，使模擬能階與實驗點對齊。  
  * 產生並儲存：  
