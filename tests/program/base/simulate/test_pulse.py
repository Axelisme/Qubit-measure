...
from lib.zcu_tools.program.base.simulate.pulse import Pulse, pulses_to_signal
...
class TestPulse(unittest.TestCase):
    ...
    def test_pulse_const_waveform(self):
        pulse_cfg = {"ch": 0, "length": 10, "style": "const"}
        pulse = Pulse(0, pulse_cfg)
        signal = pulse.get_signal(self.times)
    ...
    def test_pulse_gauss_waveform(self):
        pulse_cfg = {"ch": 1, "length": 10, "style": "gauss", "sigma": 2}
        pulse = Pulse(0, pulse_cfg)
        signal = pulse.get_signal(self.times)
    ...
    def test_pulse_with_start_time(self):
        # 測試start_t偏移
        pulse_cfg = {"ch": 0, "length": 5, "style": "const"}
    ...
    def test_pulses_to_signal_single_channel(self):
        # 兩個同通道pulse，訊號應疊加
        pulse1 = Pulse(0, {"ch": 0, "length": 10, "style": "const"})
        pulse2 = Pulse(0, {"ch": 0, "length": 10, "style": "const"})
        signals = pulses_to_signal([pulse1, pulse2], self.times)
    ...
    def test_pulses_to_signal_multi_channel(self):
        # 不同通道pulse
        pulse1 = Pulse(0, {"ch": 0, "length": 10, "style": "const"})
        pulse2 = Pulse(0, {"ch": 1, "length": 10, "style": "const"})
        signals = pulses_to_signal([pulse1, pulse2], self.times)
        self.assertIn(0, signals)
    ...
    def test_pulses_to_signal_overlap(self):
        # 測試部分重疊的pulse
        pulse1 = Pulse(0, {"ch": 0, "length": 5, "style": "const"})
        pulse2 = Pulse(3, {"ch": 0, "length": 5, "style": "const"})
        signals = pulses_to_signal([pulse1, pulse2], self.times)
