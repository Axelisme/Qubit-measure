...
from lib.zcu_tools.program.v2.ir.instructions import (
    BaseInst,
    PortWriteInst,
    RegWriteInst,
)
...
def test_w0_shadowed_by_r_wave(ctx):
    ...
    inst1 = RegWriteInst(dst=Register("w0"), src=SrcKeyword.IMM, lit=Immediate(1))
    inst2 = RegWriteInst(
        dst=Register("r_wave"), src=SrcKeyword.WMEM
    )  # Simplified for test
    block = BasicBlockNode(insts=[inst1, inst2])
...
def test_r_wave_fully_shadowed_by_individual_writes(ctx):
    ...
    inst_r = RegWriteInst(
        dst=Register("r_wave"),
        src=SrcKeyword.OP,
        op=AluExpr(Register("r_wave"), AluOp.ADD, Immediate(0)),
    )

    writes: list[BaseInst] = [
        RegWriteInst(dst=Register(f"w{i}"), src=SrcKeyword.IMM, lit=Immediate(i))
        for i in range(6)
    ]
