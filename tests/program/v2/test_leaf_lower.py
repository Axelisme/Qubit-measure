...
from zcu_tools.program.v2.modules.util import round_timestamp
...
def test_delay_emits_ir_delay() -> None:
    ...
    next_t = delay.ir_run(b, t=1.0, prog=prog)  # type: ignore[call-arg]
...
def test_soft_delay_no_ir_emitted() -> None:
    ...
    b = IRBuilder()
    prog = _ProgWithSoccfg()
    next_t = soft.ir_run(b, t=1.0, prog=prog)  # type: ignore[call-arg]

    assert next_t == pytest.approx(round_timestamp(cast(Any, prog), 0.05))
    root = b.build()
...
def test_direct_readout_ir_run() -> None:
    ...
    b = IRBuilder()
    next_t = ro.ir_run(b, t=0.5, prog=_ProgWithSoccfg())  # type: ignore[call-arg]
    ...
    assert isinstance(root.body[0], IRSendReadoutConfig)
    assert root.body[0].ch == "3"
    assert root.body[0].pulse_name == "ro"
    assert root.body[0].t == pytest.approx(0.0)
    assert isinstance(root.body[1], IRReadout)
    assert root.body[1].ch == "3"
    assert root.body[1].ro_chs == ("3",)
    assert root.body[1].pulse_name == "ro"
    # trig_offset folded into trigger t: 0.5 + 0.001
    assert root.body[1].t == pytest.approx(0.501)
...
def test_none_reset_no_op() -> None:
    ...
    b = IRBuilder()
    next_t = reset.ir_run(b, t=2.0, prog=_ProgWithSoccfg())  # type: ignore[call-arg]
