...
from zcu_tools.program.v2.ir.node import IRLoop, BlockNode, RootNode
...
def test_parse_loop_restores_range_hint():
    ...
    assert tuple(loop.range_hint) == (5, 5)
...
def test_emit_loop_preserves_range_hint_in_meta():
    ...
    start_meta = next(inst for inst in inst_list if isinstance(inst, MetaInst) and inst.type == "LOOP_START")
