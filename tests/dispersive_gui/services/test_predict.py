...
``calculate_dispersive_vs_flux`` (scqubits, heavy) is monkeypatched to a cheap stub
so the cache/passthrough wiring is tested without the real eigensolve.
"""
...
def test_predict_passes_downsampled_axis_and_caches(monkeypatch):
    ...
    monkeypatch.setattr(predict_mod, "calculate_dispersive_vs_flux", fake_calc)
