...
def test_correct_flux_from_f01_applies_small_biases_and_rejects_large_ones(
    ...
    monkeypatch.setattr(flux_mod, "FluxoniumPredictor", _FakePredictor)
...
def test_correct_flux_from_f01_uses_nearest_equivalent_flux_branch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _FakePredictor.calls = []
    monkeypatch.setattr(flux_mod, "FluxoniumPredictor", _FakePredictor)
    ...
    np.testing.assert_allclose(result.raw_fluxs, [0.49])
    np.testing.assert_allclose(result.candidate_biases, [0.0], atol=1e-15)
    np.testing.assert_allclose(result.candidate_flux_corrections, [0.0], atol=1e-15)
