...
# ---------------------------------------------------------------------------
# build_spec_tree — the STATIC skeleton twin of build_settable_tree: $type
# leaves (no live values), ref nodes with options but NO current / NO variant
# sub-tree, prefix sub-tree (duck-typing into a variant), no-match → {}. These
# walk an adapter's static cfg_spec() directly — no live root, no socket, no
# qapp — exactly the context-free path adapter.cfg_spec exercises.
# ---------------------------------------------------------------------------
...
def _freq_spec():
    ...
    return FakeFreqAdapter.cfg_spec()


def test_spec_tree_scalar_leaf_carries_type():
    from zcu_tools.gui.app.main.services.remote.path_resolver import build_spec_tree

    tree = build_spec_tree(_freq_spec())
    # 'reps'/'rounds' are plain int scalars → bare $type leaf, no value.
    assert tree["reps"] == {"$type": "int"}
    assert tree["rounds"] == {"$type": "int"}


def test_spec_tree_enum_scalar_carries_type_and_choices():
    from zcu_tools.gui.app.main.services.remote.path_resolver import build_spec_tree

    # 'nqz' (int, choices [1, 2]) lives inside the Pulse Readout variant; reach it
    # via a prefix that duck-types down the ModuleRef's allowed shapes.
    sub = build_spec_tree(_freq_spec(), "modules.readout.pulse_cfg")
    assert sub["nqz"] == {"$type": "int", "$choices": [1, 2]}


def test_spec_tree_sweep_is_typed_edge_subtree():
    from zcu_tools.gui.app.main.services.remote.path_resolver import build_spec_tree
...
def test_spec_tree_moduleref_options_no_current_no_subtree():
    from zcu_tools.gui.app.main.services.remote.path_resolver import build_spec_tree

    readout = _node(build_spec_tree(_freq_spec()), "modules.readout")
    # A static ref node advertises only the allowed variant labels — no live
    # 'current' chosen key and no variant sub-tree (static can't pick a default).
    assert set(readout) == {"$ref"}
    assert set(readout["$ref"]) == {"options"}
    assert readout["$ref"]["options"] == ["Direct Readout", "Pulse Readout"]


def test_spec_tree_deviceref_has_empty_options():
    from zcu_tools.experiment.v2_gui.adapters.onetone.flux_dep import (
        OneToneFluxDepAdapter,
    )
    from zcu_tools.gui.app.main.services.remote.path_resolver import build_spec_tree

    flux_dev = _node(build_spec_tree(OneToneFluxDepAdapter.cfg_spec()), "dev.flux_dev")
    # No live device registry in a static walk → options is empty (and no current).
    assert flux_dev == {"$ref": {"options": []}}


def test_spec_tree_omits_literal_fields():
    from zcu_tools.gui.app.main.services.remote.path_resolver import build_spec_tree

    # The Pulse Readout variant's 'type' is a LiteralSpec and 'freq' is locked to
    # a literal by FakeFreqAdapter — both immutable, so dropped from the skeleton.
    sub = build_spec_tree(_freq_spec(), "modules.readout.pulse_cfg")
    assert "type" not in sub
    assert "freq" not in sub


def test_spec_tree_prefix_no_match_returns_empty():
    from zcu_tools.gui.app.main.services.remote.path_resolver import build_spec_tree

    assert build_spec_tree(_freq_spec(), "modules.nope.field") == {}
