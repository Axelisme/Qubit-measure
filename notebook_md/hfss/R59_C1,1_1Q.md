...
```python
q3d_C = C_matrix[2, 2]
q3d_EC = zeq.C2EC(q3d_C)
print(
    "Result:",
    f"\nEC = {q3d_EC:.4f} GHz Cq = {q3d_C:.3f} fF, Cc = {C_matrix[2, 4]:.3f} fF",
    f"\nCc/Cq = {C_matrix[2, 4] / q3d_C:.4f}",
)
```

```python
print(f"proper param value: {pad_width * q3d_EC / want_q3d_EC:.3f}")
```
...
result_path = r"C:\Users\SQC\Desktop\Axel\HFSS\Fluxonium2D_R59_1Q\Lj_sweep4.csv"
...
filepath = r"C:\Users\SQC\Desktop\Axel\HFSS\Fluxonium2D_R59_1Q\S Parameter Plot 2.csv"
data = pd.read_csv(filepath)
fpts = data["Freq [GHz]"].to_numpy()
signals = data["mag(S(port_R,port_L)) []"].to_numpy()

%autoreload 2
from zcu_tools.experiment.v2.onetone import FreqExperiment
FreqExperiment().analyze(result=(fpts, signals), use_abcd=False)
```
