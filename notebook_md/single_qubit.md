...
# ml, md = em.new_flux(0.0, clone_from="032023_0.122mA", unit="A")
ml, md = em.use_flux(label="032219_2.000mA")
ml, md
```
...
```python
filename = "lookback"
savefig(fig, os.path.join(em.flx_dir, "image", f"{filename}.png"))
lookback_exp.save(
...
exp_cfg = {
    ...
    "sweep": make_sweep(7435, 7460, 1001),
...
exp_cfg = {
    "modules": {
        ...
        "readout": {
            "type": "readout/pulse",
            "pulse_cfg": {
                "waveform": ml.get_waveform("ro_waveform", {"length": 1.0}),
                "ch": md.res_ch,
                "nqz": 2,
                "freq": 0.0,  # not used
                "gain": 0.0,  # not used
...
_ = res_pdr_exp.run(soc, soccfg, cfg, earlystop_snr=100.0)
```
...
exp_cfg = {
    ...
    "sweep": {
        "flux": make_sweep(-7e-3, 7e-3, 3001),
        # "flux": make_sweep(5.0, -5.0, 101),
        # "freq": make_sweep(md.r_f - 1 * md.rf_w, md.r_f + 1 * md.rf_w, 101),
        "freq": make_sweep(7444, 7447, 101),
    },
    "relax_delay": 0.1,  # us
}
...
jpa_freq_exp = ze.jpa.JPAFreqExp()
...
jpa_flux_exp = ze.jpa.JPAFluxExp()
...
jpa_pdr_exp = ze.jpa.JPAPowerExp()
...
```python
md.cur_value = flux_yoko.set_current(1.8e-3)
...
exp_cfg = {
    ...
    "sweep": make_sweep(md.q_f - 20, md.q_f + 20, step=0.5),
    # "sweep": make_sweep(5650, 5750, step=0.5),
    "relax_delay": 0.5,  # us
}
...
```python
flx_bias = preditor.calculate_bias(md.cur_value, md.q_f)
flx_bias * 1e3
```
...
dual_reset_pdr_exp = ze.twotone.reset.dual_tone.PowerExp()
_ = dual_reset_pdr_exp.run(soc, soccfg, cfg)
```
...
dual_reset_len_exp = ze.twotone.reset.dual_tone.LengthExp()
_ = dual_reset_len_exp.run(soc, soccfg, cfg)
```
...
exp_cfg = {
    ...
    "sweep": {
        "freq": make_sweep(md.r_f - 2.5 * md.rabi_f, md.r_f + 0.5 * md.rabi_f, 51),
...
_ = bathreset_len_exp.run(soc, soccfg, cfg, detune=activate_detune)
```
...
```python
%matplotlib inline
md.best_ro_freq, fig = opt_ro_freq_exp.analyze(smooth=2)
md.best_ro_freq
```
...
```python
%matplotlib inline
md.best_ro_length, fig = opt_ro_len_exp.analyze(t0=5.0)
md.best_ro_length
```
...
```python
%matplotlib inline
md.t2r, md.t2r_err, detune, _, fig = t2ramsey_exp.analyze()
...
_ = t2echo_exp.run(soc, soccfg, cfg, detune=activate_detune)
```
...
exp_cfg = {
    "modules": {
        # "reset": "reset_120",
        "pi_pulse": ml.get_module("pi_len", {"block_mode": False}),
    ...
    "relax_delay": 5 * md.t1,  # us
    "sweep": {
        "times": list(range(5, 0, -1)),
        "length": make_sweep(5.0, 100, 101),
    },
}
cfg = ml.make_cfg(exp_cfg, reps=1000, rounds=100)
...
```python
device_cfg_path = os.path.join(em.flx_dir, "device_info.json")
...
```python
%matplotlib inline
md.confusion_matrix, md.ge_radius, fig = ge_sh_exp.calc_confusion_matrix(
...
```python
%matplotlib inline
fig = t1_sh_exp.analyze(confusion_matrix=md.confusion_matrix, skip=1)
```
...
```python
%matplotlib inline
fig = mist_sh_exp.analyze(md.g_center, md.e_center, md.ge_radius)
```
...
```python
%matplotlib inline
ac_stark_coeff, fig = ac_stark_sh_exp.analyze(
    md.chi,
    md.rf_w,
    confusion_matrix=md.confusion_matrix,
    cutoff=0.05,
...
exp_cfg = {
    ...
    "sweep": {
        "flux": make_sweep(1.55e-3, 1.65e-3, 51),
        # "flux": make_sweep(-4.0, -3.0, 201),
        "freq": make_sweep(4450, 4525, step=0.5),
    },
    "relax_delay": 1.1,  # us
}
...
```python
filename = (
    f"{qub_name}_flux_rf{cfg['init_pulse']['freq']}_gain{cfg['init_pulse']['gain']:.3f}"
)
qub_flux_exp.save(
...
```python
filename = f"{qub_name}_fastflux_twotone"
savefig(fig, os.path.join(em.flx_dir, "image", f"{filename}.png"))
lf_twotone_exp.save(
    filepath=os.path.join(database_path, f"{filename}@{em.label}"),
    # filepath=os.path.join(database_path, f"{filename}@{em.label}"),
...
exp_cfg = {
    ...
    "sweep": {
        "length": make_sweep(0.0, 0.9, 51),
        "phase": make_sweep(-360, 360, 151),
    },
    "readout_t": 0.91,
    "relax_delay": 0.1,  # us
}
...
lf_distortion_exp = ze.fastflux.DistortionExp()
_ = lf_distortion_exp.run(soc, soccfg, cfg)
```

```python
%matplotlib inline
fig = lf_distortion_exp.analyze(cfg=cfg)
```

```python
filename = f"{qub_name}_flux_distortion_gain{cfg['modules']['flux_pulse']['gain']:.3f}"
savefig(fig, os.path.join(em.flx_dir, "image", f"{filename}.png"))
lf_distortion_exp.save(
    filepath=os.path.join(database_path, f"{filename}@{em.label}"),
    # filepath=os.path.join(database_path, f"{filename}@{em.label}"),
    comment=make_comment(cfg),
)
