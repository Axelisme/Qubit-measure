...
OVERRIDE_TOOLS: dict[str, dict[str, Any]] = {
    ...
    "gui_editor_set": {
        ...
        "description": (
            ...
            "{path, value}: copy each canonical dotted leaf from gui_editor_get_cfg "
            "(sweep '<path>.<edge>', ref key '<path>.ref', ref children direct); "
            "removed '.sweep.*' / '.value.*' aliases are rejected with their "
            "replacement. 'value' is a "
    ...
    "gui_tab_set_cfg": {
        ...
        "description": (
            "Batch-set canonical cfg paths copied from gui_tab_get_cfg in order "
            "(non-atomic batch). Sweep edges use '<path>.<edge>', ref keys use "
            "'<path>.ref', and ref children descend directly; removed '.sweep.*' / "
            "'.value.*' aliases are rejected with their replacement. Apply "
            ...
            "NOT rolled back. On success returns {valid, removed, added}; path diffs "
            "are the final net before/after result (A→B→A is empty), not transient "
            "per-edit churn. A running tab is rejected (cancel the "
