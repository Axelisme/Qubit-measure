...
hand-written and fork ``script/run_fluxdep_gui.py``.

The socket/RPC/stdio plumbing lives in the shared
:mod:`zcu_tools.mcp.core.bridge` (:class:`McpBridge` + helpers); this module
keeps only fluxdep's config + the read-only ``send_gui_rpc`` wrapper + the three
lifecycle tools. Events are dropped (the agent uses request/reply, not event
...
# GUI the user drives and must not kill the user's GUI. The bridge's own stop()
# is used in _cleanup_on_exit so a server-launched GUI is not orphaned.
