...
def calc_phi(norm_yc: float, norm_r0: float) -> float:
    return -np.arcsin(np.clip(norm_yc / norm_r0, -1.0, 1.0))


def calc_Qc(Ql: float, phi: float, r0: float) -> complex:
    return Ql / (2 * r0 * np.exp(-1j * phi))


def calc_Qi(Ql: float, Qc: complex) -> float:
...
class HangerModel:
    ...
    def _refine_complex(
        ...
        *,
        ...
        phi_limit = np.pi / 2.0 - 1e-7
        initial = [
            freq_init,
            np.log(max(initializer["Ql"], 1.0)),
            np.log(max(abs(initializer["Qc"]), 1.0)),
    ...
    def fit(
        ...
        theta0 = align_phase_to_data(
            float(np.angle(-a0 * np.exp(1j * phi))),
            freqs,
            data_phases,
            freq,
        )
