...
from .delay import DelayAutoReg
...
class ImproveAsmV2(AsmV2):
    ...
    def delay_auto_by_reg(self, time_reg: str, gens=True, ros=True) -> None:
        """Auto-align to timeline, then increment by runtime cycles from a register."""
        self.append_macro(DelayAutoReg(time_reg=time_reg, gens=gens, ros=ros))
    ...
    def pulse_by_reg(
        self, ch: int, addr_regs: list[str], t: Union[float, QickParam] = 0.0
    ) -> None:
        self.append_macro(PulseByReg(ch=ch, t=t, addr_regs=addr_regs))
