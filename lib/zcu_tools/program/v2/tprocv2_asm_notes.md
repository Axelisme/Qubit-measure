...
在實作編譯器優化（如 IR 最佳化器）時，必須考慮以下隱性硬體相依：

1. **暫存器別名 (Aliasing)**
   - `r_wave` 與 `w0`~`w5` 在硬體上是連動的。修改 `r_wave` 會同時覆寫所有的 `w0`~`w5`；反之修改任何 `wN` 也會改變從 `r_wave` 讀取到的內容。

2. **隱性讀取相依**
   - **基準時間 (`s14`)**: 所有的定時指令（`WPORT_WR`, `WMEM_WR`, `DPORT_WR`, `TRIG`）不論是否顯式標註 `@T`，底層皆以 `s14` 為參考基準。若在此類指令前後挪動修改 `s14` 的指令（如 `TIME inc_ref`），將導致波形時間偏移。
   - **當前時間 (`s11`)**: `WAIT` 與 `TIME updt` 指令會隱性讀取硬體當前時間。
   - **WAIT 組合**: `WAIT` 指令會同時比較 `s11` 與 `s14`。

3. **具備副作用的寫入 (Volatile Writes)**
   - 寫入 `s0`-`s14` 通常具有即時的硬體副作用。
   - `s1`: 讀取觸發 PRNG 步進。
   - `s2`: 寫入觸發外設狀態清除。
   - `s12-s13`: 用於對 Python 端（Host）進行 I/O 通訊。
   - **DCE 警告**: 除非確認目標是通用寄存器 `rN` 或 `s15`，否則不可因為「後續無讀取」而刪除對系統暫存器的寫入指令。

4. **ALU Flags**
   - 帶有 `-uf` 標記的指令會修改隱性的狀態 Flag。若後續有 `-if(COND)` 指令依賴此 Flag，則該指令及其副作用不可被最佳化移除。
