...
**`parse_addr` 的解析優先順序**：必須是 `MemAddr（&N）→ Register（含 &rN）`，`addr` 欄位不包含 Label。通用 `parse_addr` 對非法值回傳 `None`；instruction `from_dict` 層必須把非法 `ADDR` 欄位轉成 `ValueError`，不可靜默丟欄位。
...
`ADDR_REG` 是 lowering scratch register，不可作為 dispatch selector 或 branch compare register。`IRBranch.compare_reg`、`IRDispatch.value_reg` 與 high-level `modules.control.Branch(compare_by=...)` 會拒絕 canonical name 為 `s15` 的 register（含 `s_addr` alias），避免 big-PMEM jump setup 覆寫 selector。
...
3. **`IRParser`** (`ir/factory.py`)
   ...
   - `IRBranch` 的 fallback lowering 會插入 synthetic end-of-case jump 與 trailing `end_label` block；`parse()` 讀回這種形狀時只會剝除 `can_remove=True` 且名稱符合 branch end label 形狀的 synthetic block，因此 user label 不會被誤吞。Fallback unparse 不可原地修改 case AST。
   - `_check_sese()` 會逐一追蹤每個 loop / branch / dispatch region 內定義的 labels，並掃描 region 外所有 `BaseInst.need_labels`（含 `RegWriteInst(label)`、`CallInst`、`DmemAddr.table_labels` 等）。任一 region 外部都不可跳入或引用該 region 內 label；這是 tree lowering、clone/rename 與 unroll 的前置假設。
...
- `_run_chunklist_opt`：ChunkPass 與 ChunkListPass 放**同一組**迭代（互相可觸發對方的工作）。達 `max_opt_iterations` 仍未收斂時記 `logger.warning`，**用 deepcopy probe 列出仍回報 changed 的 pass 名稱**——這是震盪訊號，非正確性問題（結果仍是合法程式，只是未達最佳），且 diagnostic 不可再次 mutate live chunks。
...
- `disable_opt=True` blocks：不可刪除、替換、關閉 `disable_opt`，也不可改變 block 內的 program-memory word 數；pipeline 對 chunk pass 與 tree pass replacement 都有 runtime 驗證。`UnreachableEliminationPass` 即使在 dead-mode 也保留 fixed block。
...
| `_parse_optional_*` helpers | typed operand or `None` | optional typed fields；key 存在但 parse 失敗時 raise |
| `_parse_wp_port(d, cmd)` | `tuple[str \| None, str \| None]` | `WP` / `PORT` 必須成對出現，拒絕 packed `WP="r_wave p0"` |
| `_parse_dport_src_keyword(val)` | `Literal["imm", "reg"]` | `DPORT_WR.SRC` 必填且只接受 QICK 合法 source |

不使用 `cast()` 壓過型別錯誤——所有窄化都應反映在真實的 parse 邏輯中。

**`DPORT_WR.DATA` strict parsing**：`DportWriteInst.from_dict()` 不走 `parse_value()` 的寬鬆 fallback。`SRC="imm"` 時 `DATA` 必須是 bare integer（`ImmValue`），`SRC="reg"` 時 `DATA` 必須是 register；缺 `SRC`、`DATA="#1"`、`DATA="garbage"` 或未知 register name 都必須在 IR parse 階段直接 `raise ValueError`，不能延後到 assembler。
...
`DeadTestEliminationPass` 不做跨 block flag liveness，所以 block 沒有 terminal branch 時必須保守保留 pending `TEST`；下一個 fall-through block 可能有 conditional instruction 消耗 flags。只有明確的無條件 terminal branch 才能證明 pending `TEST` 沒有 fall-through consumer。
