...
class IRBuilder:
    def build(self, prog_list: list[dict], labels: dict[str, str]) -> RootNode:
        ...
        loop_section_names = frozenset({"initial", "update", "stop_check", "body"})
        
        def _loop_sections(loop_node: IRLoop) -> tuple[BlockNode, BlockNode, BlockNode, BlockNode]:
            return (
                loop_node.initial,
                loop_node.update,
        ...
        for d in prog_list:
            ...
            if isinstance(inst, MetaInst):
                if inst.type == "LOOP_START":
                    loop_node = IRLoop(name=inst.name)
                    ...
                    if len(block_stack) > 1 and block_stack[-1] in _loop_sections(loop_node):
                        block_stack.pop()
                    ...
                    if not struct_stack or not isinstance(struct_stack[-1], IRLoop) or struct_stack[-1].name != inst.name:
                    ...
                    if len(block_stack) > 1 and struct_stack and isinstance(struct_stack[-1], IRBranch):
                        branch_node = struct_stack[-1]
                    ...
                    if not struct_stack or not isinstance(struct_stack[-1], IRBranch) or struct_stack[-1].name != inst.name:
