...
2. **Implicit Hardware Dependencies (隱性硬體相依)**:
   ...
   - **`TimedMergePass` 的 s14-write 屏障**：`pending TIME inc_ref` 不可越過任何**非 `TimeInst`** 且寫入 `s14` 的指令，因為那類指令會以未知方式修改時間基準，使已累積的 delta 失效。`TimeInst` 本身的所有變體（lit inc_ref、register inc_ref）都在前面的分支已處理，因此屏障條件為 `TIMED_BASE_REG in inst.reg_write and not isinstance(inst, TimeInst)`。`TIME #0`（value=0，不被 `_is_lit_time` 捕捉）跨越是安全的，正確由此條件排除。
...
- `IRLoop` lowering 採 do-while 形狀：
  ...
  - `start` label
  - body
  - cond back-edge `JUMP start -if(...) -op(counter - n)`
  - `end` label
...
- register-driven loop unroll（`_maybe_build_jump_table`）回傳 `BlockNode`，內含：
  ...
  - back-edge：`JUMP entry_0 -if(S) -op(i - n)`，exit label 為 fall-through
...
- `DATAFLOW_TRANSPARENT_INSTS` — 19 個對 dataflow tracking 透明的 instruction 型別 tuple，`DeadWriteEliminationPass` 和 `IncRegMergePass` 共用。
- `collect_referenced_labels(chunks)` 是 `analysis.py` 提供的共用 utility（Phase 17 從 `labels.py` 移入，消除 `labels.py` 對 instruction/node 型別的懶 import 循環依賴），`DeadLabelEliminationPass` 和 `BlockMergePass` 共用同一份實作；它走 `BaseInst.need_labels`，因此 dmem dispatch table 透過 `DmemAddr` 引用的 entry label 也會被視為「存活」，不被 dead-label 刪除。
...
- **`allocated` 的生命週期依用途分兩層**：`clone_renamed()` / unroll 內部仍使用函數-local `allocated` 來做單次 clone session 的 uniquify；`PipeLineContext.allocated_names` 則是單次 pipeline run 的 shared name pool，只給 tree-pass 配置新 label。IRParser 的 `allocated` 仍是 per-instance local set，不跨 pipeline run 共用。
