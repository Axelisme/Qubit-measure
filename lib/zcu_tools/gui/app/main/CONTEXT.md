...
一個進行中 async 操作（device.setup / run.start / connect.start）的 async-task 式 handle。`OperationGate` 是統合 façade，內部**責任分離**：`_OperationExclusion`（互斥，release 即移、讓出 hardware）+ `_OperationRegistry`（**所有 async task 工具的統一入口** —— per-token outcome/Event + cancel/poll/await）。生命週期綁死（一個 operation 同時是互斥的+可 await 的，同一 token 串起兩者，不拆成可選疊加），但三個 async 動詞統一在 Registry：
