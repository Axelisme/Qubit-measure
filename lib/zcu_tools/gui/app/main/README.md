**Last updated:** 2026-06-14（agent launch：Windows argv[0] 由 `resolve_agent_command()` 解析——Claude Desktop 內建 CLI 優先、再 PATH `claude`、否則 fast-fail；Windows 終端改用 conhost `cmd /c start`，避開 Store 版 wt 對 `AppData\Roaming` 的沙箱）
