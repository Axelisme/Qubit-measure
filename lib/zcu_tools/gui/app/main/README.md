**Last updated:** 2026-06-29（linked ref locked-literal projection）
...
- **連線/專案拆分**：`soc.connect`（kind+ip?/port?，**同步** RPC）與 `startup.apply`（chip/qub/res+可選 `scope_id`）皆 ParamSpec 生成；agent 端的 mock 連線走 `gui_soc_connect(kind='mock')`。`scope_id` 來自 `result_scope.list`，省略時使用/建立 `result/<chip>/<qub>` generated scope；setup apply 不再接受任意 `result_dir` / `database_path` override。
...
- **role default（`shared/defaults/role_table.py` 的 `ROLE_TABLE` 資料 + `role_blank`/`role_ref` 兩個 generic builder）**：每 role 一個 `RoleDef`（spec shape + `Pulse`/`Ro` seed + `Lib` 採用策略）。`role_blank`（md 衍生預設、不查庫）+ `role_ref`（查庫 preferred → fallback blank;**optional 查無回 `None`**——ADR-0010 的停用態（裸 `None`，取代舊 `DisabledRefValue`），與 optional spec 對稱；`fields` 型別 `dict[str, Optional[CfgNodeValue]]`）。seed 值載體 `Md`（md-linked，含 `expr` live 算術 + nested fallback）/ `TRIG`，是日後 multi-source `ValueSource` 的接縫。共用工具在 `defaults/helpers.py`（`patch_pulse_fields`/`patch_ro_cfg_fields`/`make_trig_offset`/`make_default_value`/`select_named_module_value`/`select_named_waveform_value`，builder 直接複用）。**零鎖定**（鎖歸 adapter cfg_spec）。fake/demo adapter 在 `adapters/fake/`（registry key `fake/*`，與真實驗選單分開）。
  ...
  - **形狀明確 vs role 預設**：`readout`/`reset` 本身是 library-aware 的 pulse 形狀 role（`Init.ADOPT` 查庫、`Init.INLINE` 產 fresh inline pulse——取代舊的獨立 `pulse_readout`/`pulse_reset` role），其餘形狀是獨立 role（`direct_readout`/`readout_dpm`、`none_reset`/`two_pulse_reset`/`bath_reset`）。spec 只允許單一形狀的 adapter（如 onetone 用 `make_pulse_readout_module_spec`）**掛對應形狀 role**（pulse 用 `.role(..., "readout", Init.INLINE)`），不靠 role 預設「正好是該形狀」（最小驚訝）。
  ...
  - **ref vs blank 慣例**：spectroscopy 用 `Init.INLINE`（探測是新脈衝、freq 常被 sweep 鎖 0.0）;校準後 t1/t2/rabi 用 `Init.ADOPT`（查庫）。`Lib.prefer`（preferred_names）按角色名慣例,使用者存庫時用該名。
  - （演化：舊三層 `default_*` wrapper → 每角色一檔 single-layer factory → 現收斂為 `ROLE_TABLE` 資料 + `role_blank`/`role_ref` 兩 builder。）
...
- **`align_locked_literals(spec, value)` = adapter-local value projection**：外部 value tree 進入某個 adapter spec 時顯式套用 `LiteralSpec`（例如 linked `ModuleLibrary` entry relink 到 tab、`CfgBuilder.build()` 掛 role value），會遞迴穿透 ref 並用 value 的 `type`/`style` discriminator 選 caller allowed shape；它只覆寫 locked literal leaf，不修改 library、不標記 override、不取代 `validate()`。`validate()` 仍 fast-fail，負責抓沒有經過投影或手工構造的非法 value。
...
- **無 analysis 的 adapter**：只填兩格 generic（`BaseAdapter[Cfg, Result]`，PEP 696 補 `No*`），宣告 `capabilities = AdapterCapabilities(analysis=AnalysisMode.NONE)`，**不覆寫** `analyze`/`get_analyze_params`。`analyze` 預設 body **raise NotImplementedError**（Fast Fail）——`analysis=NONE` 時 framework gate（`tab.py`）擋掉不會走到；`analysis=FIT` 卻忘覆寫 `analyze` 時才觸發 raise。`get_analyze_params` 預設則回 `params_cls()`（當 params 全 default 可建構，含 `NoAnalyzeParams`）；只有 params 有無-default 欄位卻忘覆寫時才 raise（驗證亦同步在 import 時擋）。(舊 `NoAnalysisAdapterMixin` 已刪)
...
- 預設 save path policy 由 `BaseAdapter` 提供；一般 adapter 只需定義 filename stem；`make_save_paths()` 是 pure suggestion，不建立目錄。`SaveService` 是 GUI save flow 的 path reservation boundary：在提交 worker 前用 `reserve_labber_filepath` 決定 final data path，再把 exact path 傳給 adapter/experiment save；低層 persistence 不自動 suffix 或覆寫。路徑結構對齊 notebook（single_qubit.md）：data=`{database_path}/{stem}@{active_label}`、image=`{result_dir}/exps/{label}/image/{stem}.png`。**日期段（`YYYY/MM/Data_MMDD`）在 `database_path` 裡**（`ResultScopeManager.derive_paths` 用共享純函數 `get_datafolder_path` 算出 `Database/chip/qub/YYYY/MM/Data_MMDD`，與 notebook 的 `create_datafolder` 回傳同款，唯不建目錄）——`make_default_save_paths` 直接 join `ctx.database_path`，**不再重複補日期**。`get_datafolder_path`（純）↔ `create_datafolder`（= 純 + makedirs）是 utils 層同一抽出；GUI 走純版本。result scope 由 `ResultScopeManager` 統一決定：未選既有 scope 時用 generated `result/<chip>/<qub>`，選既有 scope 時 `result_dir` 取該 `params.json` 目錄；`database_path` 仍由 chip/qub + 今天日期推導。setup dialog prefill 時對持久化的 chip/qub **re-derive**（取今天日期，避免隔天恢復寫進昨天資料夾）
