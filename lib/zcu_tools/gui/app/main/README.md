...
- **adapter 規格查詢（Phase 169 移除）**：`adapter.cfg_spec` / `adapter.analyze_spec` 兩個 wire method 已連同 `build_spec_tree` 系列 helper 移除（冗餘 agent 工具收斂）。要讀某 adapter 的 cfg 形狀或 analyze 參數，build 一個 tab 後走 `tab.list_paths`（巢狀 value tree）/ `tab.get_analyze_params`（當前 analyze 參數）。adapter 的 `cfg_spec()` classmethod 仍是 spec 層內部 API（`make_default_cfg` 用），不受影響。
