**Last updated:** 2026-06-14（agent launch：argv[0] 由 `resolve_agent_command()` 解析——PATH 上的獨立 `claude` 優先、Windows 再退 Desktop 內建、否則 fast-fail；Windows 終端直接 spawn launcher + `CREATE_NEW_CONSOLE`，避開 Store 版 wt 的 `AppData\Roaming` 沙箱與 `cmd /c start` 的引號問題）
...
- **連線/專案拆分**：`connect.start`（kind+ip?/port?）與 `startup.apply`（chip/qub/res+可選 result_dir/database_path）皆 ParamSpec 生成；`gui_connect_mock` 為可選參數的便利包裝。省略 result_dir → DRAFT context。
