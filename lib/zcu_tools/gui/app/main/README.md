...
- **adapter spec 查詢（未建 tab）**：`adapter.cfg_spec(name)`（`path_resolver.list_spec_paths` 走純 `cfg_spec()` 樹，回 shape skeleton——ModuleRef/WaveformRef 只列 `.ref` selector + allowed choices，**不穿透 variant 內部欄位**；variant 欄位走 build tab + `tab.list_paths`）；`adapter.analyze_spec(name)`（`describe_analyze_params` 反射 dataclass，`analysis=NONE` 回 `[]`）。
