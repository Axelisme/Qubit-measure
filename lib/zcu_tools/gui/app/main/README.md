...
- **cfg path 發現**：`tab.list_paths` 讀該 tab 的 `CfgEditorService` session（經 `editor_id_for_owner(tab_id)` → `cfg_editor_get`），回 `{path, kind, value, type, choices?}`；每個 path 保證可被 `editor.set_field`（同一棵 session model）設定。form 未 populate（無 session）→ `precondition_failed`。
