**Last updated:** 2026-06-22 (WIRE 37: tab.list_paths nested value tree, stage1 folds tree + guide dedup; GUI 44, MCP 51)
...
├── path_resolver.py    — resolve_and_set（dotted path → live LiveField mutation）+ `build_settable_tree`（live 樹→巢狀 value tree，見下方「巢狀 value tree 格式」）+ `list_settable_paths`（flat dotted-path 列表，供 editor.set_field diff / cfg_spec 內部使用，**不對外暴露為 tab.list_paths 回傳值**，tree 與 flat 並存）/ list_spec_paths（靜態 spec 樹,ModuleRef/WaveformRef 只列 .ref + allowed choices,不穿透 variant 內部欄位——哪個 variant 是 live 預設屬 value 層 make_default_value(ctx) 決定,static context-free 無從得知；要看 variant 欄位走 build tab + list_paths）
