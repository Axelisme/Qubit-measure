**Last updated:** 2026-06-23 (read-reveal guard refresh: a pure read in `_READ_REVEALS` refreshes only the keys it reveals; writes/unclassified reads still whole-table refresh — self-block safe; fixes the read-absorption gap, two residual gaps documented; MCP_VERSION 62, WIRE unchanged. Earlier same day — Phase 171: measure-gui MCP interface redesign — ~29 agent-tool renames via method_specs.tool_name / server overrides (wire dotted names unchanged), retire of state_check / project_info / device_reconnect + per-op poll/wait into generic gui_op_poll/gui_op_wait, semantic stage bundles gui_tab_open/run/analyze_review/commit; wire-contract changes (reply shapes + params) bump WIRE 44 / GUI 51 / MCP 59)
