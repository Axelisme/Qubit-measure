**Last updated:** 2026-06-22 (WIRE 38: adapter.cfg_spec / editor.get / editor.open reply flat→tree; cfg_spec / editor.get add prefix; editor.get drop verbosity/under; remove list_spec_paths / cfg_editor_get / paths_view / verbosity projection; GUI 45, MCP 52)
...
├── path_resolver.py    — resolve_and_set（dotted path → live LiveField mutation）+ `build_settable_tree`（live 樹→巢狀 value tree，見下方「巢狀 value tree 格式」）+ `build_spec_tree`（靜態 spec 樹→巢狀 spec tree；節點形狀：scalar `{"$type":...}`、enum `{"$type",...,"$choices":[...]}`、sweep 子樹(typed edges)、ref `{"$ref":{"options":[...]}}`(無 current、不展開 variant)、section 子樹；`prefix` 回子樹、不命中回 `{}`；取代已移除的 `list_spec_paths`）+ `list_settable_paths`（flat dotted-path 列表，**diff-only/internal**：僅供 editor.set_field diff 內部使用，**不對外暴露**）
...
- **`WIRE_VERSION`(=38,`wire_version.py`)**:mcp↔RPC **接口契約**(method 集合 / params / event 序列化)。mcp **pin 並比較**;不符 = 兩端講不同協議 → **硬 MISMATCH**。**只在契約變更時 bump**。本 app 自持(`wire_version.py`)而非共用 wire 層——每個 GUI app 各自演化其 wire 契約。WIRE 36：`tab.list_paths` 新增 optional `prefix`（dotted-path 前缀過濾，不命中回空）。WIRE 37：`tab.list_paths` reply 由 flat path 列表改為巢狀 value tree（見下方「path_resolver / path 文法」段）；移除 `verbosity` 與 `under` 參數；`gui_run_stage1` 折 `tree` 取代舊 `paths`+`cfg_summary`。WIRE 38：`adapter.cfg_spec` / `editor.get` / `editor.open` reply 由 flat 改為巢狀 tree（同 `build_settable_tree` / `build_spec_tree` 格式）；`adapter.cfg_spec` 與 `editor.get` 新增 `prefix` 參數（同 `tab.list_paths`，dotted，不命中回 `{}`）；`editor.get` 移除 `verbosity` / `under` 參數；移除 `list_spec_paths`（agent-facing）、`cfg_editor_get` 舊別名、`paths_view`、verbosity 投影；`list_settable_paths` 降為 set_field diff 的 **internal-only** helper，不對外暴露。
- **`GUI_VERSION`(=45,`wire_version.py`)**:GUI 進程**代碼版本**。**只顯示、不比較**(代碼版本是該進程自己的屬性,mcp 不該 pin)。bump 任何想能看出 reload 的 GUI 變更,**含不動 wire 的純內部變更**(拆分用意:內部變更 bump 它而非 WIRE_VERSION,契約版本不動)。
...
| `tab.get_cfg_summary` | `schema_to_raw()` → `_strip_cfg_tags()` | `{summary: clean dict}`（RPC 仍存在；WIRE 37 起 `gui_run_stage1` 折 `tree` 而非此 RPC，但 RPC 本身保留供按需查詢）|
...
| `adapter.cfg_spec` / `analyze_spec` | 靜態 cfg / analyze-params 規格（無 tab/context）；`cfg_spec` 支援 `prefix`（dotted，不命中回 `{}`）；unknown adapter → invalid_params | `build_spec_tree → {tree}` / `{params}` |
...
`$` 前綴是保留命名空間，`$`-鍵 = 葉節點元資料，無 `$`-鍵 = 普通子樹。**編輯仍走 dotted path**（`editor.set_field`，路徑語法不變）；`tab.list_paths` 是 discovery 工具，不是 edit 工具。`list_settable_paths`（flat dotted-path 列表）仍在 `path_resolver` 內部供 `editor.set_field` diff / `cfg_spec` 使用，**不作為 `tab.list_paths` wire 回傳值**。
