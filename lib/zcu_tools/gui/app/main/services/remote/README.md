**Last updated:** 2026-06-15 (remove device.active_setups; active_operations supersedes it, WIRE 26)
...
- **`GUI_VERSION`(=32,`wire_version.py`)**:GUI 進程**代碼版本**。**只顯示、不比較**(代碼版本是該進程自己的屬性,mcp 不該 pin)。bump 任何想能看出 reload 的 GUI 變更,**含不動 wire 的純內部變更**(拆分用意:內部變更 bump 它而非 WIRE_VERSION,契約版本不動)。
