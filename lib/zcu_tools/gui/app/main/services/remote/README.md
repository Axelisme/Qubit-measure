**Last updated:** 2026-06-17 (SoC-connect tool rename `gui_connect_start`→`gui_soc_connect` (MCP-only, no wire change); predictor.set_model_params — typed EJ/EC/EL model install, no params.json; WIRE 32, GUI 39, MCP 40)
...
- **mcp operation-handle 簿記**:start op 回的 `operation_id` 由 `send_gui_rpc` 經 `_OP_KEY_OF[method](params)` 捕捉進 `_OP_BY_KEY[semantic_key]`(latest wins)並從 result strip(raw id 不上 agent)。device connect/disconnect/setup 都 key 在 `device:<name>`;run→`tab:<id>`、connect.start→`soc`。agent 用語義 wait tool 翻語義名→id→`operation.await`：`gui_device_wait_operation(name)`（涵蓋 connect/disconnect/setup）、`gui_run_wait(tab_id)`、`gui_soc_connect_wait()`。
- **mcp short-wait degrade**(`_start_op_with_short_wait`，泛化):有快/慢雙模態的 start op 都套——`gui_device_connect/_disconnect/_setup`、`gui_run_start`、`gui_soc_connect`。start 後等 `wait_seconds`(預設 1.0):settle→回 `{status:"finished", **product()}`(device→`device.snapshot`、run→`tab.snapshot`、connect→`view.snapshot`+`soc.info`，各自的產物狀態;connect 額外折入 SoC 硬體摘要，`gui_soc_connect_wait` 同樣帶 description);TIMEOUT→`{status:"pending"}`(agent 改對應語義 wait：`gui_device_wait_operation`/`gui_run_wait`/`gui_soc_connect_wait`，或 watch `*_finished`/`device_changed`/`soc_changed`);genuine failed/cancelled 仍上拋。便利封裝歸 mcp 層,RPC 維持 start+await 兩原語。
