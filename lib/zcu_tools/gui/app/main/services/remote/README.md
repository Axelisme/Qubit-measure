**Last updated:** 2026-06-15 (analyze.cancel + mathtext thread-safe lock; WIRE 29, GUI 36, MCP 36)
...
| `analyze.cancel` | `cancel_analyze(tab_id)` | `{ok, cancelled}`（取消互動 analyze、清 is_analyzing 使 tab 可關；無進行中互動 analyze → graceful no-op `ok:false`）|
