**Last updated:** 2026-06-28 (result-scope setup; WIRE 48 / GUI 61 / MCP 70)

# `zcu_tools/gui/services/remote/` — RemoteControlAdapter

> **MCP entry 與 GUI-進程側 handler 分屬兩處**：MCP 端 entry 在 `zcu_tools/mcp/measure/server.py`（使用方），共用 `McpBridge`（`zcu_tools/mcp/core/bridge`）；**本目錄只留 GUI-進程側 handler**（`service`/`dispatch`/`events`/`method_specs`/`wire_version`）。`MCPBridgeConfig` 繼承共用基底 `McpServerConfig`（measure 用子類）。下文若提及 `mcp_server`，指的即是 `mcp/measure/server.py`。
...
├── path_resolver.py    — resolve_and_set（dotted path → live LiveField mutation）+ `build_settable_tree`（live 樹→巢狀 value tree，見下方「巢狀 value tree 格式」）+ `list_settable_paths`（flat dotted-path 列表，**diff-only/internal**：僅供 editor.set_field diff 內部使用，**不對外暴露**）
...
- **`WIRE_VERSION`(=48,`wire_version.py`)**:mcp↔RPC **接口契約**(method 集合 / params / event 序列化)。mcp **pin 並比較**;不符 = 兩端講不同協議 → **硬 MISMATCH**。**只在契約變更時 bump**。本 app 自持(`wire_version.py`)而非共用 wire 層——每個 GUI app 各自演化其 wire 契約。WIRE 40-43（Phase 170）：`tab.list_paths` → `tab.get_cfg`（回 value tree）+ 新增 `tab.set_cfg`（batch write）；`tab.list` → `tab.list_all`；run / analyze / save / writeback method 由頂層移至 `tab.*` 命名空間（`run.start` → `tab.run_start`、`analyze.start` → `tab.analyze`、`save.data/image/result/post_image` → `tab.save_*`、`writeback.set/apply/preview` → `tab.writeback_*`）；context md/ml prefix（`context.get_md*` → `context.md_*`、`context.get_ml*` → `context.ml_*`）；`editor.open` → `editor.new`。WIRE 44（Phase 171）：measure-gui 介面重設計——絕大多數工具改名走 MCP **tool alias**（`method_specs.tool_name` / server override），**wire dotted method 名不變**，不計入 wire bump；本 bump 只記真正的 wire 契約變更（reply shape + param）：`startup.apply` 回 resolved project dict（非 bool）、`tab.list_all` 回 named dict `{tabs, active_tab_id, running_tab_id}`（非 positional 2-tuple）、`soc.info` 新增 optional `include_cfg`（default false，省略則不回 ~2 KB cfg）、`device.list` 每項帶 fine-grained status enum（非 `is_connected:bool`）、`device.reconnect` 回 `{operation_id}`（非 `{}`）、`predictor.predict` param 改名（`value`→`device_value`、`from_lvl/to_lvl`→`from_level/to_level`）、`tab.writeback_apply` 回 enriched echo `{applied_ids, written, context_version}`。WIRE 45 新增同步 `tab.load_data`，用已開 adapter tab 載入 canonical result file。WIRE 46 新增 `arb_waveform.list` / `arb_waveform.preview` / `arb_waveform.set`。WIRE 47 新增 read-only `value.list` / `value.read`。WIRE 48 新增 `result_scope.list`，且 `startup.apply` 移除任意 `result_dir` / `database_path` override，改用 optional `scope_id`，回傳補上 `params_path` / `scope_id`。
- **`GUI_VERSION`(=61,`wire_version.py`)**:GUI 進程**代碼版本**。**只顯示、不比較**(代碼版本是該進程自己的屬性,mcp 不該 pin)。bump 任何想能看出 reload 的 GUI 變更,**含不動 wire 的純內部變更**(拆分用意:內部變更 bump 它而非 WIRE_VERSION,契約版本不動)。
- **`MCP_VERSION`(=70,`mcp/measure/server.py`)**:mcp bridge 自己的**代碼版本**。只顯示;bump 想能看出 reconnect 是否載入了 bridge 端修改的變更(**含純 mcp-side convenience 工具**,如 batch fan-out 不動 wire 但 bump MCP_VERSION)。

mcp `_wire_version_note()` 只比 wire(硬 MISMATCH),gui/mcp 版本純顯示——一致→` wire v48 (mcp==gui); gui code v61, mcp code v70.`；pre-split GUI 缺 gui_version 顯示 `v?`。**用途**:agent 改完碼 reconnect/launch 後直接看三個數字對不對,自己判斷有沒有 reload，不靠 bridge 斷言（注意 ToolSearch 顯示的 schema 是 reconnect 當下快照，可能滯後；以 banner 版本 + 實際行為為準）。**mcp tool error 末尾附 `reason: <tag>`**——precondition 失敗的 `GuiRpcError.reason`（no_run_result/no_project…，已在 wire envelope）透到 agent text，免 parse 句子分支。三個版本號的逐版演化以 git history 為準（`wire_version.py` / `mcp/measure/server.py` 不再保留 per-version 列）。

**改 wire 層後的標準驗證流程**:改完碼 → `/mcp reconnect measure-gui`(重啟 MCP server)→ 重新 `gui_launch`(拿到新 GUI 子進程)→ 看回傳的 `wire vN (mcp==gui); gui code vX, mcp code vY` —— wire 不符=MISMATCH(契約)；gui/mcp code 數字對不對自己判斷有沒有 reload(改 GUI 內部記得 bump GUI_VERSION、改 bridge 記得 bump MCP_VERSION,才看得出 reload)。注意 reconnect 會連帶讓舊 GUI 子進程失聯但**不殺它**——舊進程仍佔著 port。**`gui_launch` 有 pre-flight：port 已被佔(舊 GUI 還在)直接 fail-fast**（MCP_VERSION 6 起），明說「port 已佔，先 gui_stop / kill 舊 run_measure_gui.py，或換 port」——避免舊版的「靜默連到殘留舊進程(舊碼、卻回新 pid)」陷阱（握手版本若同版看不出，只有實際效果能分辨）。launch 預設 port 8765 期望**空閒**（起新的）；`gui_bridge_connect` 預設 8765 期望**已有 GUI 在聽**（連現有的），無 GUI 時報 `No GUI is listening on 127.0.0.1:<port>`。兩者預設對稱、語義相反。目前三版本：WIRE 48 / GUI 61 / MCP 70。

### Dialog API

- `DialogName` enum（`setup` / `device` / `predictor` / `inspect` / `startup` / `arb_waveform`）是 wire-stable 識別。
- `MainWindow._open_dialogs: dict[DialogName, QDialog]` 是唯一 registry；`open_dialog` / `close_dialog` 是 UI click 與 remote 共用的單一進入點。所有 dialog 一律 **non-modal**（`open()` not `exec()`），否則 modal 會凍住 event loop 導致 marshal deadlock。Predictor 是 persistent dialog：Close/視窗 X 只 hide，重開沿用同一 instance；`dialog.list_open` / `view.snapshot.open_dialogs` 只列 visible dialogs。
...
| `view.snapshot` | active tab、context label、status、open dialogs 的 JSON summary。**internal-only**（`_NON_GENERATED_METHODS`）：mcp `_assemble_overview` 讀 `active_tab_id`，無 agent 工具 |
...
| `tab.list_all` | `list_tab_ids` + `get_running_tab_id()` | `{tabs:[{tab_id, adapter_name, is_running}], active_tab_id, running_tab_id}`（named dict，WIRE 44；MCP tool `gui_tab_list`） |
...
| `tab.run_start` | `start_run(tab_id)` (RPC fire-and-forget；mcp `gui_tab_run_start` 加 short-wait degrade) | `{operation_id}` |
| `tab.load_data` | `load_tab_result(tab_id, data_path)`（同步；使用者先開對應 adapter tab，載入 canonical result file；不需要 SoC） | `{tab_id, data_path, result_type, has_run_result, has_cfg_snapshot, has_analyze_params, source_kind, analyze_params}` |
| `tab.load_data` failure | canonical 檔案不相容、adapter 不支援 load、檔案讀取失敗 | `precondition_failed` + `reason` (`invalid_data_file` / `unsupported_load` / `data_file_read_failed`) |
| `tab.run_cancel` | `cancel_run()` | `{ok:true}` |
...
| `tab.save_data` / `tab.save_image` / `tab.save_result` | 對應 Controller method（回寫入路徑：data 是 `.hdf5`+suffix，同步可知）| `{data_path}` / `{image_path}` / `{data_path, image_path}` |
| `tab.save_post_image` | `save_post_image(tab_id, image_path?)`（鏡像 `tab.save_image`，存 post-analysis 圖，需 post 結果）| `{image_path}` |
...
| `soc.info` | `get_soc_info(include_cfg)` → `describe_soc(soccfg)`（精簡 per-channel 表，非 QICK 原生長 description）+ `is_mock`；`include_cfg=true`（default false，WIRE 44）才加 `json.loads(dump_cfg())`；未連線→precondition_failed | `{description, is_mock}` + `cfg`（僅 include_cfg=true） |
...
| `result_scope.list` | `list_result_scopes()` 掃描 `result/**/params.json`；每個 scope 的身分由 `gui/result_scope.py` 的 `migrate_params_v0_to_v1_project_info` 解析並原地升級。canonical `project.{chip_name, qubit_name}` 優先；缺 project 時從 `result/` 下路徑推導，兩層=`chip/qubit`、一層=`name/name`、其它=`unknown/unknown`，`resonator_name` 補 `unknown` | `{scopes:[{scope_id, chip_name, qub_name, result_dir, params_path, source}], chip_names:[...]}` |
| `arb_waveform.list` | `list_arb_waveforms()`；MCP tool name 是 `list_arb_waveform`（無 `gui_` prefix） | `{waveforms:[name]}` |
| `arb_waveform.preview` | `get_arb_waveform_preview(name)`；即時 render normalized static PNG | `{recipe, preview_figure}` |
| `arb_waveform.set` | `set_arb_waveform(name, recipe, overwrite, expected_versions?)`；guarded on `arb_waveforms`，成功才回 payload，validation/collision/missing 走 typed tool error | `{success:true, status:"created"|"overwritten", preview_figure}` |
| `startup.apply` | ParamSpec(chip/qub/res, scope_id?) → `StartupProjectRequest` → `apply_startup_project`。`scope_id` 必須來自 `result_scope.list`；省略時使用/建立 generated `result/<chip>/<qub>` scope 並初始化/遷移 `params.json`。不接受任意 `result_dir` / `database_path` override。apply 無 no-op 語意——總是 mutate 或 raise | `{chip_name, qub_name, res_name, result_dir, database_path, params_path, scope_id}` |
| `device.connect` / `disconnect` | 對應 DeviceService command（回 operation token） | `{operation_id}` |
| `device.reconnect` | 對應 DeviceService command（now a tracked async op，WIRE 44；MCP 端併入 `gui_device_connect` 的 name-only 重連） | `{operation_id}` |
| `device.forget` | 對應 DeviceService command | `{}` |
...
| `device.setup_spec` | 從 live `info.model_fields` 導出 setup `updates` 可用欄位（name/type/choices(Literal→enum)/current/settable，protected type/address 標 settable=false）；device 須連線 | `{fields:[...]}` |
| `device.cancel_operation` | `cancel_device_operation(name)` | `{}` |
| `device.list` / `device.snapshot` | device summary；`list` 每項帶 fine-grained `status` enum（DeviceStatus value，WIRE 44，取代 `is_connected:bool`，與 snapshot/active_operations 投影一致）；`snapshot` 含 `info`（`BaseDeviceInfo.to_dict()`，JSON-safe scalar，agent 可讀 live 參數值如 source `value`） | `{devices:[{name, status}]}` / `{snapshot}` |
...
| `context.md_get` / `md_get_attr` / `ml_get` | scalar / name-only context query | keys / value / names |
| `value.list` / `value.read` | read-only value source lookup（resolve-once；`value.read` optional `type` ∈ int/float/str/bool，unknown/type mismatch→INVALID_PARAMS，暫不可用→PRECONDITION_FAILED，provider bug→CONTROLLER_ERROR） | `{values:[{key,type,owner,description}]}` / `{key,type,owner,description,value}` |
| `context.ml_list_roles` | `get_role_catalog().list_meta()` | `{roles:[{role_id,label,item_kind}]}` |
| `context.ml_create_from_role` | `create_from_role(item_kind, role_id, name)` 一次性憑空建（撞名 fail-fast=PRECONDITION） | `{}` |
...
| `app.shutdown` | `RenderView.request_shutdown()`(主線馬上回，singleShot 延後關閉讓 RPC reply 先送出，再 `_perform_close`)。**internal-only**（`_NON_GENERATED_METHODS`）：`gui_stop` 經 `bridge.stop(shutdown_rpc="app.shutdown")` 驅動，無 agent 工具 | `{shutting_down:true}` |
...
| `tab.analyze` | `ctrl.analyze(tab_id, updated_params)` fire-and-forget；FIT→worker，**INTERACTIVE→主線程經 View 掛互動 picker、不起 worker**（Controller `_start_interactive_analyze`，token=analyze lease，Done 才 settle）| `{operation_id}` |
...
| `tab.post_analyze` | `ctrl.start_post_analyze(tab_id, updated_params)` fire-and-forget（第二層分析，**FIT-only worker**，無 INTERACTIVE）；gate 主 analyze result 不存在→PRECONDITION（`no_analyze_result`）| `{operation_id}` |
| `tab.save_set_paths` | `update_tab_save_paths(tab_id, data_path, image_path)` | `{}` |
| `tab.save_data` / `tab.save_image` / `tab.save_result` | path optional — falls back to `get_tab_save_paths()` | 回寫入路徑（`{data_path}` / `{image_path}` / 兩者）；data 路徑 `.hdf5`+suffix 在 start 同步算好回傳，save 本身仍 async |
| `context.md_set_attr` / `md_del_attr` | 對應 ContextService MD mutation | `{}` |
| ~~`context.set_ml_module` / `set_ml_waveform`~~ | **移除（ADR-0011 / wire v4）**：ml entry 建/改一律走 editor session（`context.ml_create_from_role` + `editor.new(from_name)` → `editor.set_field`（含 EvalValue）→ `editor.commit`） | — |
| `context.ml_del_module` / `ml_del_waveform` | 對應 ContextService ML delete | `{}` |
| `context.ml_rename_module` / `ml_rename_waveform` | `(old, new)` → register(new)+delete(old)+emit ML_CHANGED 一次；clash/missing/empty→PRECONDITION。cfg 引用 old 經 ModuleRefLiveField self-heal 降級 inline Custom（不遷移名） | `{}` |
...
| `editor.new` | `open_cfg_editor(item_kind, from_name)` — **modify-only**（編輯既有 entry）；憑空建走 `context.ml_create_from_role(role_id='<disc>:blank')` 再 `editor.new(from_name)`。`discriminator` 已從 RPC 移除（內部 seam 保留） | `{editor_id, tree}`（同 `build_settable_tree` 巢狀帶值樹） |
| `editor.set_field` | `cfg_editor_set_field(editor_id, path, value)`；tagged eval `{__kind:eval,expr}` 由 service decode 成 EvalValue，tagged value ref `{__kind:value_ref,key,type?}` 立即讀 value source 並 materialize 成 direct scalar。**run-guard**：若 session owner（`owner_of_editor`）是正在 run 的 tab → `precondition_failed`（與人靠 disabled form 同義，ADR-0013 F11） | `{paths(子樹), valid}` |
| `editor.get` | `build_settable_tree(root)` → 回巢狀帶值樹；支援 `prefix`（dotted，不命中回 `{}`）；`verbosity` / `under` 已移除 | `{tree}` |
...
| `notify.await` | `await_notify(token, timeout)`（**off_main_thread**：阻塞 IO worker 等 NotifyChannel settle）| `{reason, reply?}`，`reason ∈ {reply, dismiss, timeout}`。兩者皆 `_NON_GENERATED_METHODS`——agent 只見手寫 `gui_prompt_user`（mcp 端 serial compose 此 two-RPC，timeout/dismiss 不 raise），不曝露 raw notify.*（ADR-0025）|
...
**Run 生命週期事件**：`RUN_STARTED{tab_id}` 與 `RUN_FINISHED{tab_id, outcome, error_message}`（outcome ∈ {finished, failed, cancelled}、失敗帶 error_message）—— 一事件一語義（wire v7 拆自舊 `RUN_LOCK_CHANGED`，後者用欄位有無區分 start/end 對 agent 不友善）。MainWindow 兩者都訂：start→`refresh_run_lock(tab_id)`、finish→`refresh_run_lock(None)`，鎖刷新需求由兩 handler 共同滿足，不重複 emit。cancel 判讀在 `RunService`（持 stop_event 者，ADR-0019）：`BackgroundRunner` 只回 done/failed，`_on_bg_done`/`_on_bg_error` 看 `stop_event.is_set()` 映 finished vs cancelled（+ 可能的部分結果）→ `_on_run_cancelled`；`cancel_run` 走 `gate.cancel(token)` 設 stop_event（異步通知，不阻塞）。progress bar 帶 `percent`（0–100，total 未知時 None）+ raw n/total（經 `operation.progress`，折進泛型 `gui_op_poll` running 回傳）。**Phase 129：run 一開始 `RunService.start_run` 即 `State.clear_tab_results`（清 run/analyze/figure/writeback）**——run **進行中/失敗**時 tab 無 result，analyze/save fail-fast `no_run_result`（非誤導的 no analyze params / busy）。**cancel 例外（Phase 130 釐清）**：worker 在 stop_event 前已產出的 partial result 由 `_on_run_cancelled` 保留（`has_run_result=true`、可 analyze），有意為之；完全沒產出才無 result。

**Device setup 生命週期事件（與 run 對齊，wire v10）**：`DEVICE_SETUP_STARTED{name}` + `DEVICE_SETUP_FINISHED{name, outcome, error_message}`，完全鏡像 run_started/run_finished（一事件一語義 + outcome）。進度走 `operation.progress(operation_id)`（同 run，按 id 查的 live 拉，不嵌 snapshot、不經 event；折進泛型 `gui_op_poll` running 回傳）。progress 在 `ProgressService`（非 DeviceService、非 dialog）：container 生死綁 setup operation（`make_factory`→終局 `discard_operation` 在 `_finish_operation`）→ 關 dialog 不銷毀進度（container 仍活，重開 re-attach 即見）。

**`AnalyzeResultBase`** mixin：adapter result dataclass 繼承後自動取得 `to_summary_dict()`（反射過濾非 JSON-safe 欄位）。`NoAnalysisResult` 也繼承。

**save path 設計**：`tab.save_data` / `tab.save_image` / `tab.save_result` 的路徑參數現為 optional，省略時 fallback 到 `get_tab_save_paths()`（override 優先，無 override 則 adapter 預設）。新增 `tab.save_set_paths` 讓 agent 設定 path override，等同 UI 的 path 欄位。
...
- `PredictorDialog`：已加 `PREDICTOR_CHANGED` 訂閱（load/clear 後 dialog 即時更新）；measure-gui 以 persistent host mode 開啟，hide 不會 emit `finished`，所以 hidden cache 仍保持訂閱與已算曲線。
...
### editor.set_field / tab.get_cfg path 文法
...
- Scalar：`section.sub.field` → `ScalarLiveField.set_value`；agent 可傳 tagged eval 或 tagged value_ref。GUI scalar text 不再解析 exact token，而是在 eval mode 經共用 input helper 做分段候選與空格提交：`@{key} ` 先替換成普通文字，再由 eval 欄位保存/解析。
...
#### tab.get_cfg 巢狀 value tree 格式（WIRE 37）

`tab.get_cfg` 回 `build_settable_tree` 產出的**巢狀 dict tree**（`prefix` 參數回該節點子樹；不命中回 `{}`；指向 sweep edge 節點回整個 sweep 節點）。**`verbosity` 與 `under` 參數已移除**。葉節點以 `$`-前綴鍵區分：

| 節點形狀 | 意義 |
| --- | --- |
| `<bare scalar>` | scalar 葉：裸值（`null` = 未設） |
| `{"$value": v, "$choices": [...]}` | enum/Literal 葉：`$value` 是當前選中值，`$choices` 是允許集合 |
| `{"start": ..., "stop": ..., "expts": ..., "step": ...}` | sweep 葉：裸 edge 值（子樹，無 `$` 前綴） |
| `{"$ref": {"current": "key", "options": [...]}, ...sub-tree...}` | ref 節點：`$ref.current` 是 tagged 內部 key（`set_field` 接受 bare option 名）；`$ref.options` 是可選 bare 名列表；其餘欄位是**當前選中 variant 的子樹**（只展開 current variant，其他 variant 不顯示） |

`$` 前綴是保留命名空間，`$`-鍵 = 葉節點元資料，無 `$`-鍵 = 普通子樹。**編輯仍走 dotted path**（`editor.set_field`，路徑語法不變）；`tab.get_cfg` 是 discovery 工具，不是 edit 工具。`list_settable_paths`（flat dotted-path 列表）仍在 `path_resolver` 內部供 `editor.set_field` diff 使用，**不作為 `tab.get_cfg` wire 回傳值**。
...
- **gc=True**(`editor.new`):agent 編 ml entry,無 owner。`commit`(lowering→ml)/`discard` 自動回收;斷線經 `_ClientCtx.editor_ids` 回收(`_track_editor_lifecycle` 在 open 加、commit/discard 移除;`_drop_client` marshal 到 main / `stop` 直呼 → `_reclaim_editors` → `ctrl.discard_cfg_editors`);另有**數量上限 LRU**(`_MAX_HEADLESS_EDITORS`)防孤兒。commit 失敗不銷毀,讓 agent 重試。
...
**eval-on-commit / value-ref-on-set**:scalar 可填 `{"__kind":"eval","expr":"r_f - 0.1"}`,decode 成 `EvalValue`;`set_field` 當下即經 `ScalarLiveField.set_value` 對 md 求值 fill `resolved` snapshot,故 `commit` 經 `CfgSchema.to_raw_dict` lower 成 concrete(ml 只存 concrete)時靠 snapshot,不靠 md;unresolved → fail-fast。`{"__kind":"value_ref","key":"device.flux.value","type":"float"}` 是 resolve-once：`set_field` 當下讀 `value.read` 同一套 lookup，成功後改成 `DirectValue`，後續 commit/lowering 不再追來源。`device.flux` 這裡是名為 `flux` 的 device，不是 device service 推導的全域 active flux。**commit drift 檢查**:`cfg_editor.lower` 經 `ModuleLibraryWritePort.get_current_md()` 把 md 一併傳 lowering(純為檢查,非解析依賴)——snapshot 與 md 現值不一致(field set 之後 md 變了)時 `logger.warning`,snapshot 仍勝(語義不變)。`editor.commit` 是 **upsert**(create-or-modify)語義,撞名靜默覆寫(與 create-only 的 `context.ml_create_from_role` fail-fast 區分)。
...
- **mcp 簿記(policy)**:`mcp_server` 持 `_LAST_SEEN`(guard baseline)、`_GUARD_DEPS`(write 依賴對應表:run→cfg/tab/soc/context/device:*/devices:__set__;load→tab/result/analyze/context;save→result/save_path;commit→editor/context)、`_READ_REVEALS`(read 揭露對應表,見下一 bullet)。`device:*` glob 守既有成員 mutation,`devices:__set__` 基數 key 守集合成員新增/移除(glob 對新成員失明,Phase 102)。`send_gui_rpc` 對 guarded method 由 `_build_expected_versions` 組 `expected_versions`(展開邏輯與 `_READ_REVEALS` 共用 `_expand_pattern_keys`)。async terminal(run/device/connect 完成)的版本 bump 由 agent 的 **poll/wait**（`gui_op_poll`/`gui_op_wait` 內部即 `operation.await` round-trip）承接 resync（Phase 120c-1/2 取代舊「event poll 後 refresh」）。被拒 → refresh + **`data.stale` 翻成語義訊息**給 agent（Phase 120c-3：`_describe_stale_keys`，「the active context / this tab's cfg / device 'flux'」）。
- **baseline 刷新策略(讀只刷揭露 key；寫整表,MCP_VERSION 62)**:成功 RPC 後依 method 種類分流刷 `_LAST_SEEN`——
  - **寫 / mutation(及任何沒把握的 read)→ 整表 `_refresh_versions()`**(`resources.versions` 全表,經 `_BRIDGE.send_rpc_raw` 避遞迴)。寫會 self-bump 它動到的 key,整表刷新吸收自己的 bump → agent 不會被自己的寫擋下(**self-block safe**)。整表永遠安全,只是不收窄下面的「讀吸收」窗。
  - **有把握的 pure read(列入 `_READ_REVEALS`)→ 只刷它揭露的 key `_refresh_revealed_versions()`**:整表讀但只把該 read 真正觀察到的 key 寫回 baseline,其餘 key 不動。**危害是 masking / false-ACCEPT**:若條目把 read 對到它「沒有完整揭露」的 key,就把該 key baseline 推到 current、expected==current → server 放行 → **漏擋**並發改動(安全破洞)。反之,推高 baseline 永遠**不會** false-reject(server 只在 expected 低於 current 時拒絕)。因此只放能**完整**揭露該 key 全部狀態的 read。目前已列:`tab.get_cfg`→`tab:{id}:cfg`、`editor.get`→`editor:{id}`、`device.snapshot`→`device:{name}`、`device.list`→`devices:__set__`（僅集合成員,不含 `device:*`）。**刻意留整表（均屬確定過廣或映射不確定）**:(1)**所有 `context.*` read 一律留整表**——`context` 追蹤**全部 md/ml 內容**;任何單一 read 只揭露投影(md_get=key list、md_get_attr=單值、ml_get=name list、ml_list_roles=靜態 role catalog 與內容無關),對到 `context` 是**確定過廣**,會遮蔽 read 沒見到的並發 md/ml 改動。`context.labels`/`context.active` 連 context key 都沒有(切 context 不 bump)。(2)`value.list` / `value.read` 可投影 context/device/predictor 等不同來源，沒有固定完整 resource key 對應。(3)`tab.snapshot`(揭露多 facet、key 映射不確定)。(4)analyze/post-analyze 讀(facet↔key 映射不確定)。(5)`device.list` 已移除 `device:*`——list 只回 status,`device:{name}` 也在 info/value 改時 bump,刷 `device:*` 會遮蔽 info 改動。connect / stale-reject 仍整表。
  - **兩個殘留(誠實標)**:(a) 兩次操作間插入**無關的寫**仍會整表吸收一個並發改動(比舊「任何 RPC 都吸收」窄,非零);(b) `_LAST_SEEN` 是 **process-global、跨 agent 共享**,無 per-agent 隔離(平台無 agent 身分,per-agent cursor upstream Not Planned)→ 多 agent 並發寫同 GUI **不保證** per-agent staleness。
- **通知面（Phase 120c-2）**:GUI EventBus 照常 push 全部 event 上 wire，但 mcp 端只放行 diagnostic（resource-change event 丟棄）。agent 不再「訂閱+poll 收什麼變了」；resource 變動靠 guard 撞牆告知（語義 stale），async 完成靠 poll/wait。agent-facing 行為**無變化**(agent 看不到版本、stale-reject 訊息不變),故 run-measure-gui SKILL 不需改。
...
- `notify.await`(WIRE 30)消費 per-prompt `NotifyChannel`(獨立 event 詞彙 Reply/Dismiss/Timeout，鏡像 OperationChannel 四不變式但不混進 operation 事件集；dialog 是 timeout SSOT)。agent 經手寫 `gui_prompt_user` serial compose `notify.open`(主線 mint+開 dialog)+ `notify.await`。**嚴格契約同上**:off-main 只做 thread-safe 等待，producer(dialog 回呼/QTimer)在主線。
- **mcp operation-handle 簿記**:start op 回的 `operation_id` 由 `send_gui_rpc` 經 `_OP_KEY_OF[method](params)` 捕捉進 `_OP_BY_KEY[semantic_key]`(latest wins)並從 result strip(raw id 不上 agent)。device connect/disconnect/setup 都 key 在 `device:<name>`;run→`tab:<id>`。**`soc.connect` 不在此列**——它是同步 RPC，無 operation handle、無 `_OP_KEY_OF` 項、`operation.await/poll` 也不再認 "soc" key。agent 用泛型 wait tool `gui_op_wait(handle)` 翻 handle→`operation.await`（START reply 直接帶 handle，涵蓋 device connect/disconnect/apply 與 run）。
- **mcp short-wait degrade**(`_start_op_with_short_wait`，泛化):有快/慢雙模態的 start op 都套——`gui_device_connect/_disconnect`、`gui_device_apply`、`gui_tab_run_start`。start 後等 `wait_seconds`(預設 1.0):settle→回 `{status:"finished", **product()}`(device→`device.snapshot`、run→`tab.snapshot`，各自的產物狀態);TIMEOUT→`{status:"pending"}`(agent 改泛型 `gui_op_wait(handle)`，或 watch `*_finished`/`device_changed`);genuine failed/cancelled 仍上拋。便利封裝歸 mcp 層,RPC 維持 start+await 兩原語。**`gui_soc_connect` 不走此 degrade**——它直接同步呼叫 `soc.connect`（帶顯式 ~2s timeout，讓 board 端 1s COMMTIMEOUT 先觸發乾淨錯誤），回 `{status:"finished", soc:{description, is_mock}}`，無 `_wait`/`_poll`。
...
`tab.run_start` RPC reply OK immediately（不等 run_finished）。理由：若 RPC 等到 finish 會佔住 socket thread 數分鐘，與 30s timeout 衝突。**mcp 端** `gui_tab_run_start` 則加 short-wait degrade（快 run 直接回 tab snapshot、慢 run 回 handle）。慢 run 追蹤：泛型 `gui_op_wait(handle)`（blocking）或 `gui_op_poll(handle)`（非阻塞，running 時回傳自帶 live progress bars）。
...
**Batch convenience 工具（mcp-side fan-out，不動 wire）**:`gui_editor_set`（一個 editor 套多個 `{path,value}`）與 `gui_context_md_write`（多個 `{key,value}` MetaDict attr）是 `_OVERRIDE_TOOLS` 裡的迴圈封裝，內部逐筆呼叫**既有** `editor.set_field` / `context.md_set_attr` wire RPC——**不新增 wire method，故 WIRE_VERSION 不動**（與 piggyback 同類「純 mcp 端 convenience」）。Phase 171 E5 起這兩個工具 **batch-only**（單筆 set 不再有獨立 agent 工具，wire method 仍在）。語義刻意 **fail-fast 且非原子**：第一筆失敗即停、先前已套用的**不 rollback**、raise 點名失敗的 path/key 與已套用筆數（`applied_count`/`failed_index`），讓 agent 自行 reconcile（符合專案 Fast-Fail 風格；真正原子性需 RPC 層交易支援，目前不提供）。`gui_editor_set` 成功回 `{applied, valid}`（套用筆數 + 結果 draft 是否合法），**刻意不 echo cfg 內容**（echo 會逼一次 lowering，eager 求值 EvalValue）；要看 cfg 走 `gui_tab_get_cfg`。入參在任何 RPC 發出前先驗證 shape（`_coerce_pairs`，空 list / 缺 key 直接 `ValueError`），把失敗邊界鎖在「nothing applied」。
...
Setup 只發**兩個** event：`DEVICE_SETUP_STARTED`（worker 啟動）+ `DEVICE_SETUP_FINISHED`（terminal，帶 outcome）。**進度不經 event**（走 `operation.progress(operation_id)` 拉 live model，折進泛型 `gui_op_poll` running 回傳），所以沒有舊版「大量 progress step 塞爆 event queue」的問題——早期的 `_emit_setup_changed` 節流（每 20 步）已隨之移除（progress SSOT 重構 + device↔run 對齊）。
