**Last updated:** 2026-06-28 (value_ref cfg edits; WIRE 47 / GUI 60 / MCP 69)
...
| `startup.apply` | ParamSpec(chip/qub/res, result_dir?, database_path?) → `StartupProjectRequest` → `apply_startup_project`。**省略 result_dir/database_path → handler 用 `derive_project_paths(chip,qub,cwd)` 填預設 per-qubit 根**(`<cwd>/result\|Database/<chip>/<qub>`，= setup dialog 填名字時預填的)→ runnable，非 DRAFT。setup dialog(UI)保留 empty=DRAFT 互動路徑。**WIRE 44：回 resolved project dict（非 bool）；apply 無 no-op 語意——總是 mutate 或 raise** | `{chip_name, qub_name, res_name, result_dir, database_path}` |
