...
| `editor.open` | `open_cfg_editor(item_kind, from_name)` — **modify-only**（編輯既有 entry）；憑空建走 `ml.create_from_role(role_id='<disc>:blank')` 再 open(from_name)。`discriminator` 已從 RPC 移除（內部 seam 保留） | `{editor_id, tree}`（同 `build_settable_tree` 巢狀帶值樹） |
...
#### tab.list_paths 巢狀 value tree 格式（WIRE 37）

`tab.list_paths` 回 `build_settable_tree` 產出的**巢狀 dict tree**（`prefix` 參數回該節點子樹；不命中回 `{}`；指向 sweep edge 節點回整個 sweep 節點）。**`verbosity` 與 `under` 參數已移除**。葉節點以 `$`-前綴鍵區分：
...
`$` 前綴是保留命名空間，`$`-鍵 = 葉節點元資料，無 `$`-鍵 = 普通子樹。**編輯仍走 dotted path**（`editor.set_field`，路徑語法不變）；`tab.list_paths` 是 discovery 工具，不是 edit 工具。`list_settable_paths`（flat dotted-path 列表）仍在 `path_resolver` 內部供 `editor.set_field` diff 使用，**不作為 `tab.list_paths` wire 回傳值**。
