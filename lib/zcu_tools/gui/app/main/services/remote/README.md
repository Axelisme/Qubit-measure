**Last updated:** 2026-06-28 (value_ref cfg edits; WIRE 47 / GUI 59 / MCP 69)
...
- **`GUI_VERSION`(=59,`wire_version.py`)**:GUI 進程**代碼版本**。**只顯示、不比較**(代碼版本是該進程自己的屬性,mcp 不該 pin)。bump 任何想能看出 reload 的 GUI 變更,**含不動 wire 的純內部變更**(拆分用意:內部變更 bump 它而非 WIRE_VERSION,契約版本不動)。
...
- Scalar：`section.sub.field` → `ScalarLiveField.set_value`；agent 可傳 tagged eval 或 tagged value_ref，GUI scalar text 只在精確 `@{source.key}` 語法下解析 value_ref
...
**eval-on-commit / value-ref-on-set**:scalar 可填 `{"__kind":"eval","expr":"r_f - 0.1"}`,decode 成 `EvalValue`;`set_field` 當下即經 `ScalarLiveField.set_value` 對 md 求值 fill `resolved` snapshot,故 `commit` 經 `CfgSchema.to_raw_dict` lower 成 concrete(ml 只存 concrete)時靠 snapshot,不靠 md;unresolved → fail-fast。`{"__kind":"value_ref","key":"device.active_flux.value","type":"float"}` 是 resolve-once：`set_field` 當下讀 `value.read` 同一套 lookup，成功後改成 `DirectValue`，後續 commit/lowering 不再追來源。**commit drift 檢查**:`cfg_editor.lower` 經 `ModuleLibraryWritePort.get_current_md()` 把 md 一併傳 lowering(純為檢查,非解析依賴)——snapshot 與 md 現值不一致(field set 之後 md 變了)時 `logger.warning`,snapshot 仍勝(語義不變)。`editor.commit` 是 **upsert**(create-or-modify)語義,撞名靜默覆寫(與 create-only 的 `context.ml_create_from_role` fail-fast 區分)。
