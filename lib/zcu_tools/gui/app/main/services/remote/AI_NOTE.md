...
| `view.screenshot` | window 或 active tab → base64 PNG；off-screen tab → `precondition_failed` |
...
| `connect.start` | ParamSpec(kind, ip?, port?) → `coerce_connect_request` → `start_connect` | `{}` |
...
| `device.list` / `device.snapshot` / `device.active_operation` | device summary；`snapshot` 含 `info`（`BaseDeviceInfo.to_dict()`，JSON-safe scalar，agent 可讀 live 參數值如 source `value`） | `{devices}` / `{snapshot}` / `{active_operation}` |
| `device.active_setup` | `get_active_device_setup()` | `{active_setup: {device_name} \| null}`（只名出哪個 device 在 setup；進度走 operation.progress(operation_id)）|
| `operation.await` | `await_operation(operation_id, timeout)` | `{status}`；blocking 等該 operation settle；failed/cancelled→PRECONDITION_FAILED（攜 `reason=outcome.status`，poll 端讀此分 cancelled/failed）、timeout→TIMEOUT。off_main_thread。mcp-internal（agent 走語義 wait 工具）|
...
| `tab.get_current_figure` | `take_figure_screenshot(tab_id)` → grab `plot_stack.currentWidget()`（當前顯示的圖：run 2D map 或 analysis fit）| `{png_b64, bytes}` or `{saved_to, bytes}` |
...
| `predictor.info` | `get_predictor_info()` | `{info: {path, flux_bias} \| null}` |
...
`MethodSpec.off_main_thread`(`BoundMethod` 同名 property 轉發)標記 blocking 等待型 handler 在 **IO worker thread** 直接跑,**不** marshal 上 main thread。唯一用途:`operation.await`。若上主線,`evt.wait()` 阻塞會卡死 main event loop,而它等的 worker terminal signal 正需 event loop 處理 → 死鎖到 timeout。
...
- `operation.await`(Phase 95)只 `OperationHandles.await_outcome(operation_id, timeout)` 等 per-token `threading.Event`(main thread terminal slot `handles.settle(token, outcome)` 寫 outcome + set Event、off-main waiter 讀 outcome；ADR-0019 把 handle 從舊 `OperationGate` facade 拆成 sibling)。取代 Phase 93 的 by-name `wait_setup_done`。
- **mcp operation-handle 簿記**:start op 回的 `operation_id` 由 `send_gui_rpc` 經 `_OP_KEY_OF[method](params)` 捕捉進 `_OP_BY_KEY[semantic_key]`(latest wins)並從 result strip(raw id 不上 agent)。device connect/disconnect/setup 都 key 在 `device:<name>`;run→`tab:<id>`、connect.start→`soc`。agent 用語義 wait tool 翻語義名→id→`operation.await`：`gui_device_wait_operation(name)`（涵蓋 connect/disconnect/setup）、`gui_run_wait(tab_id)`、`gui_connect_wait()`。
- **mcp short-wait degrade**(`_start_op_with_short_wait`，泛化):有快/慢雙模態的 start op 都套——`gui_device_connect/_disconnect/_setup`、`gui_run_start`、`gui_connect_start`。start 後等 `wait_seconds`(預設 1.0):settle→回 `{status:"finished", **product()}`(device→`device.snapshot`、run→`tab.snapshot`、connect→`view.snapshot`+`soc.info`，各自的產物狀態;connect 額外折入 SoC 硬體摘要，`gui_connect_wait` 同樣帶 description);TIMEOUT→`{status:"pending"}`(agent 改對應語義 wait：`gui_device_wait_operation`/`gui_run_wait`/`gui_connect_wait`，或 watch `*_finished`/`device_changed`/`soc_changed`);genuine failed/cancelled 仍上拋。便利封裝歸 mcp 層,RPC 維持 start+await 兩原語。
