**Last updated:** 2026-06-10 | **Commit:** a8ea8e86
...
- **MCP 端**：`McpBridge`（class，socket state 是 instance 屬性）持連線/送 RPC/reader thread/RID 路由/pending map/pid files，注入 config + `on_event` hook。各 app 的 `zcu_tools/mcp/<app>/server.py` 是 thin entrypoint（config + bridge + lifecycle tools）；measure-gui 例外，在 bridge 上額外疊 guard bridge / operation tracking / 22 override tools / 自己的 stdio loop（measure-gui 專屬 policy，不走共用 `run_stdio_loop`）。
