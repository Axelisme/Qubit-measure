...
A ``GuiProgressBarChannel`` (a QObject living on the main thread) owns the signal;
the factory it hands out builds bars bound to that channel. Updates are throttled
to avoid flooding the event loop on a fast inner loop.
"""
