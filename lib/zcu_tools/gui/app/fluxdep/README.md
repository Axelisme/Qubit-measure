...
- **`services/`** — 薄包裝純運算，mutate State：`load`(LoadService)、
  ...
  `notebook.analysis.fluxdep.processing` + `notebook.persistance`。
...
- **OneTone** `find_peaks` 只 0.1ms，**不 worker 化**；它的拖動卡是 redraw 整張
  figure，對症優化是**重用 scatter(set_offsets) + debounce redraw(50ms)**。
