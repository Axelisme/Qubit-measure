**Last updated:** 2026-06-23（ro_optimize wavelet landscape smoothing）
...
├── background.py    — 純 off-main 執行器 BackgroundService（組合共用 BackgroundRunner；`submit(work,*,run_in_pool,on_done,on_error)` 無 scopes 參數、不認 facet，ADR-0026 §2）。scope（pbar / ActiveTask）由 caller 的 work-thunk closure 套；autofluxdep worker 不畫圖故從不用 `figure_ambient`、device-setup thunk 用 session 層 `progress_ambient`
