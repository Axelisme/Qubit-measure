**Last updated:** 2026-07-22 — shared edelay branch discovery
...
## 已知坑 / 設計決策
- **preprocess edelay 共用 utils branch discovery + GUI-local numba refinement**：`find_edelay_branch` 先把所有 flux row 的相鄰 unit-phasor coherence 聚合，只搜尋一次共用 cable-delay branch；`services/_fast_edelay.py` 的 `@njit(parallel=True, fastmath=True, cache=True)` 再把 (n_flux × 1000-grid) local circle refinement 編成機器碼、per-flux 外層 `prange` 平行，內聯 **Kasa 代數圓擬合**（2×2 solve，取代 utils 的 `fit_circle_params` scipy.eig）。等距頻率軸的每 row 精修結果會對齊到 shared seed 最近的 alias，再由 preprocess 取 median，避免稀疏 grid 把相鄰 alias 線性聚合。numba 放 GIL → 不 fork、不 pickle；代表性 64 × 301 nonuniform warm run 約 0.11s，進度條維持 **busy/indeterminate**。scalar 與 fast path 共用 global branch contract，只容許 local Kasa/scipy circle grid 的離散誤差。
