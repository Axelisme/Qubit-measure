...
分層：`app.py`（composition root，bootstrap 走共用 `run_qt_app`/`gui/run_app`，`run_app` 只做 factory 接線）→ `state.py`（被動 DispersiveState + VersionTable）→ `controller.py`（命令 façade，emit EventBus；**繼承共用 `BaseController`**/`gui/controller_base` 取 state/bus/project_root + `_emit`，façade body 各 app）→ `services/`（純 sync Qt-free）→ `services/remote/`（read-only RPC + MCP）→ `ui/`（**單流程面板**）→ `event_bus.py`。
