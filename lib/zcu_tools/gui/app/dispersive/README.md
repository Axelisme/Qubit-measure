...
agent 只觀測、user 在 GUI 驅動。method set 全純查詢（state.check / project.info / fit_inputs.info / preprocess.status / fit.result{has_result,g,bare_rf,res_dim} / resources.versions），**無 mutating RPC**。`mcp_server.py` 是 **thin entrypoint over `McpBridge`**：config + bridge + 3 lifecycle tools（launch/connect/disconnect，**無 stop 曝露**），其餘 read-only tools 從 METHOD_SPECS 自動生成。**event-push 丟棄**（read-only 無診斷流，bridge on_event 不接）。預設 control port **8767**（避開 fluxdep 8766）。當 script 跑：絕對 import（無 parent package）+ `parents[6]=lib`/`parents[7]=repo`。**WIRE=3/GUI=3**（v3：fit.result 去 step）。`_h_project_info` / `_h_state_check` 使用 `gui/project.py` 的 `project_info_payload` / `is_real_project`（與 fluxdep 共用，兩 app wire 形態永遠同步）。
