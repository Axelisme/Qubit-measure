...
2.0, and forwards calls to the live dispersive GUI's ``RemoteControlAdapter`` over a
...
(``dispersive_launch`` / ``dispersive_connect`` / ``dispersive_disconnect``) are
hand-written and fork ``script/run_dispersive_gui.py``. ``tool_dispersive_stop`` exists
...
_GUI_PID_FILE = Path(gettempdir()) / "zcu_tools_dispersive_gui.pid"
_GUI_LOG_FILE = Path(gettempdir()) / "zcu_tools_dispersive_gui_debug.log"
...
def _send_line(payload: Dict[str, Any]) -> None:
    if _GUI_SOCK is None:
        raise RuntimeError("GUI not connected. Call dispersive_connect first.")
...
def tool_dispersive_connect(arguments: Dict[str, Any]) -> str:
    ...
    try:
        ...
        sock.close()
        raise RuntimeError(
            f"No GUI is listening on 127.0.0.1:{port} ({exc}). dispersive_connect "
            f"attaches to an already-running GUI; start one with dispersive_launch."
        ) from exc
    sock.settimeout(1.0)
    _GUI_SOCK = sock
    _READER_STOP.clear()
    _READER_THREAD = threading.Thread(
        target=_reader_loop, name="mcp-dispersive-reader", daemon=True
    )
    ...
    if token:
        ...
        return (
            f"Connected to dispersive-fit-gui on 127.0.0.1:{port} with token auth."
            + _wire_version_note()
        )
    return (
        f"Connected to dispersive-fit-gui on 127.0.0.1:{port}." + _wire_version_note()
    )
...
def tool_dispersive_launch(arguments: Dict[str, Any]) -> str:
    ...
    port = int(arguments.get("port", 8767))
    ...
    run_gui = repo_root / "script" / "run_dispersive_gui.py"

    if not run_gui.exists():
        raise FileNotFoundError(f"run_dispersive_gui.py not found at {run_gui}")

    if _port_is_open(port):
        raise RuntimeError(
            f"Port {port} is already in use — a GUI is likely already running "
            f"there. Use dispersive_connect to attach to it, or launch on a "
            f"different port."
        )
    ...
    if not ready:
        return (
            ...
            "call dispersive_connect manually when ready."
        )

    log_note = f" DEBUG log: {_GUI_LOG_FILE}"
    if auto_connect:
        tool_dispersive_connect(
...
def tool_dispersive_stop(arguments: Dict[str, Any]) -> str:
    global _GUI_PROC
    ...
    # No app.shutdown RPC for dispersive; close via OS signal (process group). Send
    ...
    if not exited:
        return (
            ...
            f"Re-run dispersive_stop, or pass timeout_kill=true to force-kill."
        )
...
_OVERRIDE_TOOLS: Dict[str, Dict[str, Any]] = {
    "dispersive_connect": {
        ...
        "description": (
            "Connect the MCP bridge to an ALREADY-RUNNING dispersive-fit-gui's TCP "
            "control port (default 8767). Errors if no GUI is listening there — "
            "use dispersive_launch to start one. Skip this if you used dispersive_launch "
    ...
    "dispersive_launch": {
        ...
        "description": (
            "Launch the dispersive-fit-gui as a NEW subprocess on a TCP control port "
            "(default 8767), wait until ready, and optionally connect. Use as the "
    ...
    # NOTE: there is deliberately no "dispersive_stop" entry — see _OVERRIDE_NAMES.
}

# dispersive_stop is intentionally NOT exposed as an agent tool: the agent observes a
# GUI the user drives and must not kill the user's GUI. tool_dispersive_stop is kept
...
def _cleanup_on_exit() -> None:
    try:
        tool_dispersive_stop({"timeout_kill": True})
    except Exception:
        pass
            ...
            if method == "initialize":
                resp = {
                    ...
                    "result": {
                        ...
                        "capabilities": {"tools": {}},
                        "serverInfo": {
                            "name": "dispersive-fit-gui-control",
                            "version": "1.0.0",
                        },
