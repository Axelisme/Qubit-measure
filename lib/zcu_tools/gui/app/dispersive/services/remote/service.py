"""RemoteControlAdapter — dispersive-fit-gui's second View (driving adapter).

The RPC face onto the dispersive ``Controller``, peer to the Qt ``MainWindow``
...
keeps only dispersive's *dispatch policy + domain*:
  ...
  - ``_dispatch_on_main``: marshal the handler onto the Qt main thread via the
    shared :class:`MainThreadDispatcher` and wait (timeout-bounded). dispersive
    has no version guard, no editor sessions, no off-main handlers — a bare
    marshal;
  - EventBus push: subscribe the dispersive payload types, serialize each event,
...
from zcu_tools.gui.app.dispersive.event_bus import EventBus, Payload
...
class _ClientCtx:
    """dispersive's per-connection semantic state (attached to ``link.app_ctx``).
...
class RemoteControlAdapter:
    """Driving adapter: an NDJSON RPC face onto the dispersive ``Controller``.
    ...
    def __init__(self, controller: "Controller", opts: ControlOptions) -> None:
        ...
        self._endpoint = NdjsonRpcEndpoint(
            ...
            server_name="DispersiveRemoteServer",
            router=self,
        )
    ...
    def on_client_close(self, link: ClientLink, *, on_main_thread: bool) -> None:
        # dispersive holds no per-connection resources beyond the subscription
        # set (which dies with the link); nothing to reclaim.
