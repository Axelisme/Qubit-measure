...
over plain TCP. This is the dispersive-fit-gui's *second View* (driving adapter): the
RPC face onto the ``Controller``, peer to the Qt ``MainWindow``.

Compared to measure-gui's adapter this is deliberately leaner — dispersive has no
...
handlers — so it offers only:
  ...
  - EventBus push (subscribe the 5 dispersive payload types, serialize and push to
    subscribed clients);
...
class RemoteControlAdapter:
    ...
    def start(self) -> int:
        ...
        self._thread = threading.Thread(
            target=self._serve_forever, name="DispersiveRemoteServer", daemon=True
        )
        self._thread.start()
        logger.info(
    ...
    def _accept_one(self, sel: selectors.BaseSelector, token_required: bool) -> None:
        ...
        writer = threading.Thread(
            ...
            args=(csock, state),
            name=f"DispersiveRemoteWriter[{peer}]",
            daemon=True,
        )
