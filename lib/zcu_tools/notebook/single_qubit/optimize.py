...
def optimize_1d(xs: np.ndarray, snrs: np.ndarray, xlabel: str = None) -> float:
    """
...
def optimize_ro_len(ro_lens: np.ndarray, snrs: np.ndarray, t0: float = 1.0) -> float:
    snrs = np.abs(snrs)
...
def optimize_2d(xs, ys, snr2D, xlabel=None, ylabel=None):
    """
...
def ge_lookback_show(Ts, signals_g, signals_e, *, pulse_cfg=None, smooth=None):
    """
