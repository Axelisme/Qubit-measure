...
**Last updated:** 2026-07-11 — canonical role shapes
...
`adapters/_support/`，單一實驗的 policy 留在原檔，避免 support package 變成無邊界工具箱。
...
調整 seed 或 default-value policy 時，更新 `tests/experiment/v2_gui/adapters/_support/_role_default_golden.json`
...
機制由 `adapters/_support/writeback_helpers.py` 的 `reset_module_writeback_items` 統一：
