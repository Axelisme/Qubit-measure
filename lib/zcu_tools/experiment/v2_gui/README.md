...
**Last updated:** 2026-06-25（canonical result load）
...
adapter 必須 override `load()` 或讓預設路徑以明確 `NotImplementedError` fast-fail。第一版
load 不把 `result.cfg_snapshot` 反填回 Config tab；`cfg_snapshot is None` 時 module
writeback 維持 graceful skip。
