...
新 Schedule 寫法由 `ProgramBuilder.build_and_acquire()` / `run_program(...)` 自動注入 `sched.is_stop` checker；direct ProgramBuilder path 在 stop / `KeyboardInterrupt` / acquire error 時會保留目前 buffer partial result，並把狀態寫入 `Schedule.outcome`（`completed` / `stopped` / `interrupted` / `failed`）。executor leaf 使用傳入的 `ScheduleStep`，因此 stop 與 outer loop 共用同一個 `StopSignal`；executor retry 耗盡或中斷時回傳目前累積的 partial result，並寫入 `last_run_outcome`。中斷以 round-level 粒度生效（每個 round 結束後檢查），不影響正在執行中的 round。
