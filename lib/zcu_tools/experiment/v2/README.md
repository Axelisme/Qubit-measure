...
- **grouped first adopters**：`CPMG_Exp` 使用 roles `lengths` / `signals`，axes 為 inner-first 的 `Time Index`、`Number of Pi`，盤上 `lengths` 單位為 seconds，記憶體內仍回復為 us。RO auto-optimize 使用 roles `readout_freq` / `readout_gain` / `readout_length` / `snr`；JPA auto-optimize 使用 roles `jpa_flux` / `jpa_freq` / `jpa_power` / `jpa_phase` / `snr`。三者的 runtime `load()` 都只接受 complete grouped HDF5；legacy `.npz` 或 sidecar 不由 runtime 讀取，需用 `script/migrate_experiment_data.py` 轉換。
- **auto-optimize grouped roles**：RO auto-optimize 使用 roles `readout_freq` / `readout_gain` / `readout_length` / `snr`；JPA auto-optimize 使用 roles `jpa_flux` / `jpa_freq` / `jpa_power` / `jpa_phase` / `snr`。頻率與時間在 disk 上使用 SI units（Hz、s），typed loader 重建回既有 Result 記憶體單位（MHz、us）；JPA phase 是 integer index。舊 `_params` / `_signals` / `_phases` sidecar 只屬 migration input，不由 runtime `load()` 讀取。
