...
class JPAOptimizer:
    """
    Optimizer for JPA parameters using scikit-optimize (Bayesian Optimization).
    """
    ...
    def __init__(
        ...
        flx_arr = sweep2array(flx_sweep)
        fpt_arr = sweep2array(fpt_sweep)
        pdr_arr = sweep2array(pdr_sweep)

        # Define search space
        self.dimensions = [
            Real(flx_arr.min(), flx_arr.max(), name="flux"),
            Real(fpt_arr.min(), fpt_arr.max(), name="freq"),
            Real(pdr_arr.min(), pdr_arr.max(), name="power"),
        ]

        # Initialize optimizer
        # Using n_initial_points=min(20, total_points // 4) to ensure some random exploration
        n_initial_points = min(20, max(5, total_points // 5))
        self.n_initial_points = n_initial_points
        self.opt = Optimizer(
            dimensions=self.dimensions,
            base_estimator="GP",
            acq_func="gp_hedge",  # Gaussian Process with Hedge aggregation
            n_initial_points=n_initial_points,
        )

        self.last_suggested: Optional[List[float]] = None
        self.results: List[
    ...
    def phase(self) -> int:
        """
        Return current optimization phase for compatibility with visualization.
        1 = Initial random exploration
        2 = Bayesian optimization
        """
        if len(self.opt.Xi) < self.n_initial_points:
            return 1
        return 2
    ...
    def next_params(
        ...
        # Report result from previous iteration
        if last_snr is not None and self.last_suggested is not None:
            # scikit-optimize minimizes, so we minimize negative SNR
            # We need to tell the optimizer about the point we just measured
            # However, skopt.Optimizer.ask() returns the point, and then we expect to tell() it.
            # Here the loop structure is: ask -> measure -> tell (in next call)

            # Store result for history/plotting
            self.results.append((*self.last_suggested, last_snr))  # type: ignore

            try:
                self.opt.tell(self.last_suggested, -last_snr)
            except ValueError:
                # Sometimes tell fails if point is duplicated or other issues,
                # though skopt usually handles duplicates by default now (or warns)
                pass
        ...
        return tuple(suggested)  # type: ignore
    ...
    def get_optimal_params(self) -> Tuple[float, float, float]:
        ...
        # Find best result from collected data (minimized objective)
        best_idx = np.argmin(self.opt.yi)
        best_params = self.opt.Xi[best_idx]

        return tuple(best_params)  # type: ignore
