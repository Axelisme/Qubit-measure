...
class T2RamseyTask(
    MeasurementTask[
        FluxDepCfg, FluxDepEnv, T2RamseyResult, T2RamseyPlotDict, NDArray[np.float64]
    ]
):
    def __init__(
        self,
    ...
    def run(
        ...
        if success:
            ...
            info.update(
                ...
                smooth_t2r=0.5
                * (info.last_or("smooth_t2r", t2r, task_name=task_name) + t2r),
            )
    ...
    def update_plotter(
        ...
        if flux_idx is None:
            raise ValueError("T2 Ramsey plot update requires an outer flux index")
