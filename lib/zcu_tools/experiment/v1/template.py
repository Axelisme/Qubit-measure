...
ResultFn = Callable[[int, List[ndarray], List[ndarray]], ndarray]
Signal2RealFn = Callable[[ndarray], ndarray]
UpdateCfgFn = Callable[[Dict[str, Any], int, Any], None]


def raw2signal(ir: int, sum_d: List[ndarray], sum2_d: List[ndarray]) -> ndarray:
...
def sweep_hard_template(
    ...
    *,
    ...
    with liveplotter as viewer:
        ...
        def callback(ir: int, sum_d: List[ndarray], sum2_d: List[ndarray]) -> None:
            nonlocal signals
            signals = result_fn(ir, sum_d, sum2_d)
...
def sweep1D_soft_template(
    ...
    *,
    ...
    result2signals: Optional[Callable] = None,
    signal2real: Signal2RealFn = take_signal_abs,
    progress: bool = True,
    catch_interrupt: bool = True,
        ...
        try:
            ...
            with AsyncFunc(viewer.update) as async_draw:
                ...
                for i, x in enumerate(xs_tqdm):
                    ...
                    results = measure_fn(cfg, None)
                    if result2signals:
                        signals[i], _ = result2signals(*results)
                    else:
                        signals[i] = results
        ...
        try:
            ...
            with AsyncFunc(viewer.update) as async_draw:
                ...
                for i, x in enumerate(xs_tqdm):
                    ...
                    def callback(
                        ir: int, sum_d: List[ndarray], sum2_d: List[ndarray]
                    ) -> None:
                        nonlocal _signals2D, avgs_tqdm
                        avgs_tqdm.update(max(ir - avgs_tqdm.n, 0))
                        avgs_tqdm.refresh()

                        _signals2D[i] = result_fn(ir, sum_d, sum2_d)
    ...
    with liveplotter as viewer:
        try:
            ...
            with AsyncFunc(viewer.update) as async_draw:
                ...
                for i, x in enumerate(xs_tqdm):
                    x_updateCfg(cfg, i, x)
                    ys_tqdm = tqdm(ys, smoothing=0, disable=not progress, leave=False)
                    for j, y in enumerate(ys_tqdm):
                        y_updateCfg(cfg, j, y)

                        # set again in case of change
                        set_flux(
                            cfg["dev"]["flux_dev"], cfg["dev"]["flux"], progress=False
                        )
            ...
            viewer.update(xs, ys, signal2real(signals2D))
            if "xs_tqdm" in locals():
                xs_tqdm.close()
            if "ys_tqdm" in locals():
                ys_tqdm.close()
