...
- **平行 agent 協調**：一般單 agent 工作直接在目前 checkout 完成；需要多 agent / 長線 orchestration 時，使用 `orchestrate` skill 的 `.agent_state/` worktree protocol，由 orchestrator 以 `task-id` 錨定計劃、以 `lane-id` 錨定同 task 內的平行 worktree，建立、指派、整合並關閉 worktree lane；多 task 可同時存在，但主 checkout merge 透過 merge queue 序列化（見下方「### 平行 agent 協調」）。
