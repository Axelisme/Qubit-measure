...
```bash
.venv/bin/python .claude/skills/planning-with-files/scripts/session-catchup.py "$(pwd)"
```

4. 若要建立新 plan，執行：

```bash
.claude/skills/planning-with-files/scripts/init-plan.sh <task-id> "任務目標"
```

5. 若要切換 active plan：

```bash
.claude/skills/planning-with-files/scripts/set-active-plan.sh <task-id>
```
...
```bash
.claude/skills/planning-with-files/scripts/check-complete.sh
```
