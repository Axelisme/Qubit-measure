...
<repo-python> <skill-dir>/scripts/workflow.py --root <main-checkout> merge retry-final <task-id> --requested-by <agent-id> --expect-target <recorded-7-to-40-hex-sha>
...
- `merge retry-final <task-id> --requested-by <same-agent-id> --expect-target <recorded-7-to-40-hex-sha>` — 只重試 queue head 的 `action=final`、`blocked_kind=final_fast_forward_failed`。命令要求 main HEAD仍等於entry記錄的`base_head`，並重驗requester、task/root/branch identity、recorded/expected/current integration target、clean/no-merge、untracked overwrite protection、fast-forward可行性與兩次ref snapshot；成功直接完成fast-forward並移除queue head與task。其它failure kind與legacy missing/null provenance均Fast Fail。
...
- **final fast-forward operation failure**：只有 main仍停在entry記錄的`base_head`、integration branch仍停在同一個pinned target、且blocked provenance明確為`final_fast_forward_failed`時，才以原本的requested-by與明確`--expect-target`執行`merge retry-final`。命令不做refresh或preview；成功直接完成同一target的fast-forward並釋放queue/關閉task。retry operation或postcondition再次失敗時仍保留`final_fast_forward_failed`，不改寫成其它provenance。refresh、preview、preflight、postcondition、manual與legacy blocked entry不可走此路徑。
...
- final fast-forward operation failure只用窄幅`merge retry-final`；它要求同一queue head/requester、`action=final`、`blocked_kind=final_fast_forward_failed`、recorded base/target仍完全一致，以及clean/no-merge/untracked/fast-forward與穩定雙ref snapshot。成功直接fast-forward並移除queue/task；不得用它恢復refresh、preview、preflight、postcondition、manual或legacy block。
