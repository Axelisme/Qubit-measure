...
#
# Usage:  bash .claude/skills/run-autofluxdep-gui/sync_skills.sh
set -euo pipefail

SKILL_REL="skills/run-autofluxdep-gui"
FILES=(SKILL.md)
DESTS=(.agent .codex)

# Repo root = three levels up from this script (.claude/skills/run-autofluxdep-gui).
...
echo "synced run-autofluxdep-gui to: ${DESTS[*]}"
