...
`action` 可用 `preview` 或 `final`；`status` 可用 `queued`、`merging`、`blocked`。`--requested-by <agent-id>` 用穩定的 kebab-case 標籤（建議 `orchestrator-<主題或日期>` 形式），同一 task 的所有 queue 操作沿用同一個 id，方便日後排查 stale entry 時辨識來源。使用規則：
