...
完成、blocked或needs_decision時立即以event回報；不要等待poll。單次wait timeout不是failure。不得spawn sub-agent。
Report預設30行內，raw logs寫指定artifact；主report只保留failure、evidence與next action。
